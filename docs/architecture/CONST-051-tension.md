...
## Sources verified 2026-05-29: https://go.dev/ref/mod — Cross-referenced this doc's Go-module claims against the latest official Go Modules Reference. Confirmed accurate as of this date: (a) the `replace` directive accepts a local relative path (`./` or `../`) to a replacement module root containing a `go.mod`, with the version omitted (the `replace digital.vasic.eventbus => ../dependencies/...` pattern in §"After Tasks #254/#273"); (b) two different VCS repositories declaring the SAME module path cannot both be resolved by the Go module system — if two or more modules in the build list provide a package the `go` command reports an error (the `digital.vasic.docprocessor` cross-org collision in §"Attempted fix" and Option B caveat); (c) `go.work` workspace files and the `use` directive (relative path to a module's `go.mod`) behave as described in §"Attempted fix" and Option F, and the reference notes `go.work` is typically not committed to VCS. Negative finding: none — every module-system mechanism this doc relies on is current and accurate. Go version authority per CLAUDE.md §3.1: inner module `go 1.26`, root `go 1.25.2` (confirmed in-tree); no build/Docker version pins appear in this doc so none required correction.
