...
## Sources verified 2026-05-29: https://github.com/nicksnyder/go-i18n , https://go.dev/ref/mod — Cross-referenced against the latest official `nicksnyder/go-i18n` docs (latest release **v2.6.1**, 2026-01-01) and the Go modules reference. Confirmed accurate as of this date: (a) the YAML/TOML bundle schema in §4 — singular messages use the `other:` key and plural messages use CLDR forms (`one`/`few`/`other`) — matches the upstream go-i18n message-file schema; (b) `NewLocalizer(bundle, langs...)` accepts an ordered list of language preferences (§4 / §9 locale-detection note) per upstream API. Negative finding / staleness note: this repo's `helix_code/go.mod` pins `github.com/nicksnyder/go-i18n/v2 v2.5.1` (verified in-tree), one minor version behind upstream v2.6.1 — the schema/API facts this doc documents are unchanged between those versions, so no instruction here is stale, but the dependency pin is a candidate for a routine `go get -u` bump (out of scope for a docs-only round). Go version authority per CLAUDE.md §3.1: inner module `go 1.26`, root `go 1.25.2` (both confirmed in the respective `go.mod` files); this is a non-build/non-Docker doc so no version pins required correction.
