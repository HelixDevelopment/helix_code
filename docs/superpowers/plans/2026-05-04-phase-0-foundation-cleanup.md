...
| `CONSTITUTION.md` (root) | Append Article XII §12.1 (CONST-041) + §12.2 (CONST-042) |
| `CLAUDE.md` (root) | Add CONST-041/042 sections; fix §3.2 bluff (`HelixCode/ ← SUBMODULE` → `← TRACKED SUBDIRECTORY`) |
| `AGENTS.md` (root) | Add CONST-041/042 sections |
| `CRUSH.md` (root) | Add anti-bluff Article XI §11.9 anchor + CONST-041/042 |
| `QWEN.md` (root) | Same |
...
| `scripts/verify-governance-cascade.sh` | Extend `MANDATORY_PATTERNS` with CONST-041 and CONST-042 sentinels |
...
- **No `--force`** anywhere in this plan. CONST-042 absolute.
- **No secrets in git** anywhere in this plan. CONST-041 absolute.
...
Per CONST-041, no credential artefact may be committed.
...
```bash
{
  ...
  echo "# .gitignored). NEVER commit real values. CONST-041 absolute."
  echo "#"
...
# Per CONST-042: no force push without explicit user approval.
...
# Per CONST-042: pre-push hook is a courtesy gate; constitution is the contract.
...
```bash
# Install local git hooks (CONST-042 enforcement)
./scripts/install-git-hooks.sh
```
...
git commit -m "$(cat <<'EOF'
feat(P0-09): add pre-push hook for force-push protection (CONST-042)
...
### Article XII §12.1 (CONST-041) — No-Secret-Leak
...
### Article XII §12.2 (CONST-042) — No-Force-Push
...
for f in HelixCode/HelixCode/{CONSTITUTION,CLAUDE,AGENTS}.md; do
  ...
  grep -c "CONST-041\|No-Secret-Leak" "$f" 2>&1 | head -1
  grep -c "CONST-042\|No-Force-Push" "$f" 2>&1 | head -1
done
```
...
$(for f in HelixCode/HelixCode/{CONSTITUTION,CLAUDE,AGENTS}.md; do
  printf "  %s — anti-bluff:%d  CONST-041:%d  CONST-042:%d\n" \
    "$f" \
    "$(grep -c '11.9\|tests do execute' "$f")" \
    "$(grep -c 'CONST-041\|No-Secret-Leak' "$f")" \
    "$(grep -c 'CONST-042\|No-Force-Push' "$f")"
done)
EOF
...
Each file carries Article XI §11.9 (anti-bluff), CONST-041 (no-secret),
CONST-042 (no-force-push) plus Go-specific repo addenda.
...
## Task 11: Add Article XII (CONST-041 + CONST-042) to root `CONSTITUTION.md` (P0-11 / spec P0-10)
...
```bash
grep -nE "Article XII|CONST-041|CONST-042" CONSTITUTION.md
echo "---"
...
- §12.1 (CONST-041) — No-Secret-Leak
- §12.2 (CONST-042) — No-Force-Push without explicit per-operation approval
...
## Task 12: Cascade CONST-041/042 + anti-bluff backfill to root sister files (P0-12)
...
for f in CLAUDE.md AGENTS.md CRUSH.md QWEN.md; do
  printf "%-12s anti-bluff:%d  CONST-041:%d  CONST-042:%d\n" "$f" \
...
Expected from earlier audit: CLAUDE/AGENTS already have anti-bluff (count ≥1) but lack CONST-041/042. CRUSH/QWEN lack all three.
...
git commit -m "$(cat <<'EOF'
docs(P0-12): cascade CONST-041/042 + backfill anti-bluff to root sisters

CLAUDE.md / AGENTS.md / CRUSH.md / QWEN.md all now carry the canonical
Article XI §11.9 (anti-bluff) and Article XII §12.1/§12.2 (CONST-041/042)
...
- [ ] **Step 14.2: Extend `MANDATORY_PATTERNS` with CONST-041 and CONST-042**
...
Cascading anti-bluff + CONST-041 + CONST-042 anchors from the HelixCode
...
Verifier extended patterns: CONST-041, CONST-042 added.
...
git commit -m "$(cat <<'EOF'
docs(P0-14): cascade CONST-041/042 + anti-bluff to all owned-by-us submodules

Extends verify-governance-cascade.sh to require CONST-041 and CONST-042
...
- CONST-041 / CONST-042 numbering consistent across Tasks 10, 11, 12, 14.
...
- Each push is non-force, pre-authorised per CONST-042.
