...
| `scripts/verify-llmsverifier-pin-parity.sh` | Compare `submodules/llms_verifier` SHA vs. `helix_agent/LLMsVerifier` SHA; fail on divergence |
...
# Fail if submodules/llms_verifier and helix_agent/LLMsVerifier
# point at different SHAs. Wired into make ci-validate-all.

set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

CANONICAL_PATH="submodules/llms_verifier"
...
Expected: `OK: LLMsVerifier pin parity — both at <sha>` and `exit=0`. If pins diverge (possible since `submodules/llms_verifier` was last bumped independently), the script exits 1 with the diff. In that case, decide which SHA to converge on and bump the divergent one to match before continuing — see Step 4.3.
...
echo "submodules/llms_verifier:"
git -C submodules/llms_verifier log -1 --format='%H %ci %s'
...
echo "exit=$?"
# Restore
git -C submodules/llms_verifier checkout "$PARENT_SHA"
./scripts/verify-llmsverifier-pin-parity.sh
echo "exit=$?"
```
...
Fails if submodules/llms_verifier (canonical Go-import pin)
...
modules:
  ...
  helix_libs:
    ...
    - name: LLMsVerifier
      ...
      pin: submodules/llms_verifier (canonical) + helix_agent/LLMsVerifier (transitive)
