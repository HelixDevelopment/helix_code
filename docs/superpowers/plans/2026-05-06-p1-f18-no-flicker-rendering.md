...
8. **Custom ANSI vs library (Q1=A) — non-obvious call** (recorded in spec §2 trailer): no new deps + small render surface + `applications/terminal-ui/` already uses `tview/tcell` for the full TUI; adding `bubbletea` here would be a third paradigm. The renderer is ~300 lines of pure Go; a library would be 50 KLOC.
