...
if command -v llama-server >/dev/null 2>&1 || [ -n "${HELIX_LLM_CHUTES_KEY:-}" ]; then
  GOMAXPROCS=2 nice -n 19 ./bin/helixllm --mode=full &
  PID=$!; sleep 30
  curl -fsSk https://localhost:8443/v1/health || echo "WARN: health check failed"
...
|-----------|-----------------|
| Upstream (this module imports) | Agentic, Auth, BackgroundTasks, Cache, Challenges, Concurrency, Containers, ConversationContext, Database, DebateOrchestrator, Embeddings, EventBus, Formatters, LLMOrchestrator, LLMProvider, MCP_Module, Memory, Messaging, Models, Observability, Optimization, Planning, RAG, Security, SkillRegistry, Streaming, ToolSchema, VectorDB |
