# Constitution of HelixLLM

**Version:** 1.0.0 | **Repository:** `github.com/HelixDevelopment/HelixLLM` | **Module:** `dev.helix.helixllm`

---

## Preamble

HelixLLM is the LLM abstraction layer providing a multi-provider fallback chain with local llama.cpp as guaranteed last-resort fallback. It serves OpenAI-compatible and Anthropic-compatible APIs.

This Constitution is the supreme law of the HelixLLM submodule. All other governance documents (`CLAUDE.md`, `AGENTS.md`) cascade from this Constitution and MUST NOT weaken or override any article herein.
...
- §3.2: **Submodule dependencies.** HelixLLM declares its upstream and downstream submodule dependencies explicitly. Changes to integration seams MUST be contract-tested.
...
## Article VI: HelixLLM-Specific Mandates

- §6.1: The Gateway MUST dispatch through gateway.Completer (FallbackChain), NEVER directly to Brain providers.
- §6.2: Local llama.cpp is the GUARANTEED last-resort fallback and MUST be pinned at the end of the chain.
- §6.3: TLS 1.3 is REQUIRED with self-signed cert including SANs.
- §6.4: The ScorerBridge MUST refresh provider scores every 5 minutes from LLMsVerifier.
- §6.5: Multi-model fleet routing MUST use hardware auto-profiling at boot.
...
Any submodule that imports HelixLLM MUST inherit HelixLLM's testing and anti-bluff requirements for all integration points.
...
*End of Constitution of HelixLLM*
