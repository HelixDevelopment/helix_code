...
**Submodule:** HelixCode  
**Repository:** `github.com/HelixDevelopment/HelixCode`  
**Mission:** The HelixCode submodule provides core code generation, transformation, and analysis capabilities for the HelixAgent ecosystem. It is the canonical reference for code quality standards and serves as the template for all sibling submodules.

HelixCode is governed by `Constitution.md` (supreme law). This file (`CLAUDE.md`) is the AI agent operating manual that cascades from the Constitution. `AGENTS.md` defines agent capabilities and integration patterns.
...
HelixCode is a Go module (dev.helix.helixcode, Go 1.26+) providing code generation, AST manipulation, and static analysis services. Key packages: pkg/generator, pkg/transform, pkg/analyze, pkg/verify, internal/llm. Key interfaces: generator.Engine, transform.Transformer, analyze.Analyzer.

---

## Build & Development Commands

make build, make test-unit, make test-integration, make test-challenges, make ci-validate-all

---

## Testing

make test-unit, make test-integration, make test-e2e, make test-challenges, make test-coverage
...
test -x bin/helixcode || { echo "FAIL: binary not built"; exit 1; }
GOMAXPROCS=2 nice -n 19 go test -count=1 -race -p 1 -v ./pkg/generator/...
```
...
|-----------|-----------------|
| Upstream (this module imports) | LLMProvider, DocProcessor, LLMsVerifier |
...
HelixCode serves as the canonical reference implementation. Changes here MUST be propagated to all sibling submodules that share patterns.
