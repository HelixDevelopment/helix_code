...
**Repository:** `github.com/HelixDevelopment/VisionEngine`  
**Mission:** VisionEngine provides computer vision, image analysis, and multimodal processing capabilities for the HelixAgent ecosystem. It integrates with LLM providers for vision-model-based analysis.

VisionEngine is governed by `Constitution.md` (supreme law). This file (`CLAUDE.md`) is the AI agent operating manual that cascades from the Constitution. `AGENTS.md` defines agent capabilities and integration patterns.
...
VisionEngine (dev.helix.visionengine, Go 1.26+) provides: image analysis and captioning, object detection and classification, OCR and text extraction, multimodal request routing, vision model provider integration. Key packages: pkg/analyze, pkg/detect, pkg/ocr, pkg/multimodal, pkg/providers.
...
make test-unit, make test-integration, make test-vision
...
```bash
cd VisionEngine && make build
GOMAXPROCS=2 nice -n 19 go test -count=1 -race -p 1 -v -run 'TestImageAnalysis|TestObjectDetection|TestOCR' ./tests/integration/...
```
...
|-----------|-----------------|
| Upstream (this module imports) | LLMProvider |
| Downstream (these import this module) | HelixAgent |
...
Vision model capabilities MUST be verified through LLMsVerifier. All providers in the vision fallback chain MUST implement the multimodal interface consistently.
