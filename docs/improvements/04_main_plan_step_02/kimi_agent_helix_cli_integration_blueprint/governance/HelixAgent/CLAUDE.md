...
HelixAgent is governed by `Constitution.md` (supreme law). This file (`CLAUDE.md`) is the AI agent operating manual that cascades from the Constitution. `AGENTS.md` defines agent capabilities and integration patterns.
...
HelixAgent is a Go ensemble LLM service (dev.helix.agent, Go 1.26) with 8 application entry points: helixagent (port 8100), api, grpc-server, mcp-bridge (8103), cognee-mock, sanity-check, generate-constitution, audit. Core packages: internal/llm/providers/ (51+ providers), internal/services/, internal/handlers/, internal/debate/, internal/mcp/adapters/.

---

## Build & Development Commands

make build, make build-all, make release, make docker-build

---

## Testing

make test-unit, make test-integration, make test-e2e, make test-security, make test-stress, make test-challenges, make test-coverage, make ci-validate-all
...
```bash
make build
test -x bin/helixagent || { echo "FAIL"; exit 1; }
./bin/helixagent &
for i in $(seq 1 60); do curl -fsS http://localhost:8100/v1/health >/dev/null 2>&1 && break; sleep 1; done
...
|-----------|-----------------|
| Upstream (this module imports) | HelixLLM, HelixMemory, HelixSpecifier, LLMsVerifier, Containers, Challenges, DocProcessor, HelixQA, LLMOrchestrator, LLMProvider, VisionEngine |
| Downstream (these import this module) | none (root project) |
