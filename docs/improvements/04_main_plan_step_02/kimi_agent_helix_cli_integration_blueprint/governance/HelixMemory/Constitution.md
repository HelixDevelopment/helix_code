# Constitution of HelixMemory

**Version:** 1.0.0 | **Repository:** `github.com/HelixDevelopment/HelixMemory` | **Module:** `digital.vasic.helixmemory`

---

## Preamble

HelixMemory is a unified cognitive memory engine that fuses Mem0, Cognee, and Letta into a single orchestrated system with Graphiti temporal awareness. It implements the digital.vasic.memory MemoryStore interface.

This Constitution is the supreme law of the HelixMemory submodule. All other governance documents (`CLAUDE.md`, `AGENTS.md`) cascade from this Constitution and MUST NOT weaken or override any article herein.
...
- §3.2: **Submodule dependencies.** HelixMemory declares its upstream and downstream submodule dependencies explicitly. Changes to integration seams MUST be contract-tested.
...
## Article VI: HelixMemory-Specific Mandates

- §6.1: The Fusion Engine MUST perform 3-stage fusion: Collection -> Deduplication (cosine sim >= 0.92) -> Cross-Source Re-Ranking.
- §6.2: Sleep-time compute (consolidation) MUST be enabled by default with 30m interval.
- §6.3: All memory backends MUST have circuit breakers.
- §6.4: The deduplication threshold of 0.92 MUST be configurable via HELIX_MEMORY_FUSION_DEDUP_THRESHOLD.
...
Any submodule that imports HelixMemory MUST inherit HelixMemory's testing and anti-bluff requirements for all integration points.
...
*End of Constitution of HelixMemory*
