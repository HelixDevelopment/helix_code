# Constitution of LLMProvider

**Version:** 1.0.0 | **Repository:** `github.com/HelixDevelopment/LLMProvider` | **Module:** `dev.helix.llmprovider`

---

## Preamble

LLMProvider defines the canonical LLM provider interface and shared provider implementations used across the HelixAgent ecosystem. It is the abstraction layer that enables seamless provider swapping.

This Constitution is the supreme law of the LLMProvider submodule. All other governance documents (`CLAUDE.md`, `AGENTS.md`) cascade from this Constitution and MUST NOT weaken or override any article herein.
...
## Article VI: LLMProvider-Specific Mandates

- §6.1: The LLMProvider interface is the SOLE contract for all LLM backends. All providers MUST implement Complete, CompleteStream, HealthCheck, GetCapabilities, and ValidateConfig.
- §6.2: No provider-specific code MAY leak outside the provider package.
- §6.3: All provider implementations MUST be tested with REAL API calls in integration tests.
- §6.4: The generic OpenAI-compatible provider MUST serve as the fallback for providers without dedicated implementations.
...
Any submodule that imports LLMProvider MUST inherit LLMProvider's testing and anti-bluff requirements for all integration points.
...
*End of Constitution of LLMProvider*
