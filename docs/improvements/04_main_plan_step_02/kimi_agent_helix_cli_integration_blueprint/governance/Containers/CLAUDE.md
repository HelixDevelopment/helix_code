...
Containers (digital.vasic.containers, Go 1.24+) provides: Docker/Podman auto-detection, remote distribution via SSH, compose resource normalization, capability-aware placement (GPU, storage, memory, network), partitioned distribution (no duplicates). Key packages: pkg/orchestrator, pkg/scheduler, pkg/envconfig, pkg/compose, pkg/placement.
...
make test-unit, make test-integration, make test-compose, make test-placement
...
| Downstream (these import this module) | HelixAgent, all other submodules |
