...
**Repository:** `github.com/vasic-digital/Challenges`  
**Mission:** The Challenges submodule provides the Challenge script framework for real-life use case validation across the entire HelixAgent ecosystem. Challenge scripts are the primary defence against 'tests pass, product broken'.

Challenges is governed by `Constitution.md` (supreme law). This file (`CLAUDE.md`) is the AI agent operating manual that cascades from the Constitution. `AGENTS.md` defines agent capabilities and integration patterns.
...
Challenges (digital.vasic.challenges, Go 1.24+) provides: challenge script framework, shell and Go-native challenge runners, dependency graph resolution, result aggregation and reporting. Key packages: pkg/runner, pkg/aggregator, pkg/validator, scripts/.
...
make test-unit, make test-integration, make test-challenges
...
| Downstream (these import this module) | LLMsVerifier, HelixQA |
...
Challenge scripts are the REGRESSION GUARD. Every bug fix MUST include a challenge script that reproduces the bug before the fix and passes after.
