# Constitution of Challenges

**Version:** 1.0.0 | **Repository:** `github.com/vasic-digital/Challenges` | **Module:** `digital.vasic.challenges`

---

## Preamble

The Challenges submodule provides the Challenge script framework for real-life use case validation across the entire HelixAgent ecosystem. Challenge scripts are the primary defence against 'tests pass, product broken'.

This Constitution is the supreme law of the Challenges submodule. All other governance documents (`CLAUDE.md`, `AGENTS.md`) cascade from this Constitution and MUST NOT weaken or override any article herein.
...
- §3.2: **Submodule dependencies.** Challenges declares its upstream and downstream submodule dependencies explicitly. Changes to integration seams MUST be contract-tested.
...
## Article VI: Challenges-Specific Mandates

- §6.1: Every Challenge script MUST exit non-zero when the tested feature is broken.
- §6.2: Wrapper scripts MUST use robust exit-code logic — ! grep -qs '|FAILED|' '$LOG' style.
- §6.3: Process substitution (< <(...)> ) is REQUIRED over pipes for variable state propagation.
- §6.4: Subshell-based patterns that silently drop failure state are FORBIDDEN.
- §6.5: All challenges MUST call RecordAction() for auditability.
...
Any submodule that imports Challenges MUST inherit Challenges's testing and anti-bluff requirements for all integration points.
...
*End of Constitution of Challenges*
