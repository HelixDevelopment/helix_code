...
**Repository:** `github.com/vasic-digital/LLMsVerifier`  
**Mission:** LLMsVerifier is the single source of truth for LLM provider verification, monitoring, and optimization. It runs 20+ capability tests including the 'Do you see my code?' verification and generates unified CLI agent configurations for 48 agents.

LLMsVerifier is governed by `Constitution.md` (supreme law). This file (`CLAUDE.md`) is the AI agent operating manual that cascades from the Constitution. `AGENTS.md` defines agent capabilities and integration patterns.
...
LLMsVerifier (digital.vasic.llmsverifier, Go 1.24+) structure: llm-verifier/ (core app with cmd, api, providers, verification, pkg/cliagents, database), sdk/ (multi-language SDKs), mobile/ (mobile apps). Key components: Verification Engine (20+ tests), Provider Service (17+ providers), CLI Agents Package (48 agents).

---

## Build & Development Commands

make build, make build-all, make build-acp, go build -o bin/llm-verifier ./cmd

---

## Testing

make test, make test-integration, make test-e2e, make test-coverage, make bench
...
```bash
cd LLMsVerifier/llm-verifier && GOMAXPROCS=2 nice -n 19 go test -count=1 -race -v -run 'TestModelVerification' ./verification/...
```
...
The pkg/cliagents/ package is the CENTRAL AUTHORITY for CLI agent config generation. Any agent config change MUST go through this package.
