...
**Submodule:** HelixSpecifier  
**Repository:** `github.com/HelixDevelopment/HelixSpecifier`  
**Mission:** HelixSpecifier is the Spec-Driven Development Fusion Engine. It fuses SpecKit (7-phase SDD), Superpowers (TDD + parallel subagents), and GSD (milestone lifecycle) into a unified workflow engine.

HelixSpecifier is governed by `Constitution.md` (supreme law). This file (`CLAUDE.md`) is the AI agent operating manual that cascades from the Constitution. `AGENTS.md` defines agent capabilities and integration patterns.
...
HelixSpecifier (digital.vasic.helixspecifier, Go 1.24+) architecture: User Request -> Effort Classifier -> Ceremony Scaler -> FusionEngine (SpecKit Pillar + Superpowers Pillar + GSD Pillar) -> Spec Memory -> CLI Agent Adapters. Key packages: pkg/intent, pkg/ceremony, pkg/engine, pkg/speckit, pkg/superpowers, pkg/gsd.

---

## Build & Development Commands

go build ./..., make build, make test, make fmt

---

## Testing

make test-unit, make test-integration, make test-e2e, make test-security, make test-stress, make test-bench
...
```bash
cd HelixSpecifier && GOMAXPROCS=2 nice -n 19 go test -count=1 -race -p 1 -v -run 'TestE2E_QuickFixWorkflow|TestE2E_MediumFeatureWorkflow|TestE2E_LargeImplementationWorkflow' ./tests/e2e/...
```
...
The FusionEngine supports dependency injection for DebateFunc and EffortClassifierFunc. Without injection, phases produce placeholder output with 0.75 quality score.
...
*End of CLAUDE.md — HelixSpecifier*
