# Constitution of HelixAgent

**Version:** 1.0.0 | **Repository:** `github.com/HelixDevelopment/HelixAgent` | **Module:** `dev.helix.agent`

---

## Preamble

HelixAgent is the ensemble LLM service that fronts 50+ LLM providers with dynamic selection driven by LLMsVerifier verification scores. It exposes OpenAI-compatible APIs and serves as the root orchestrator for all submodules.

This Constitution is the supreme law of the HelixAgent submodule. All other governance documents (`CLAUDE.md`, `AGENTS.md`) cascade from this Constitution and MUST NOT weaken or override any article herein.
...
- §3.2: **Submodule dependencies.** HelixAgent declares its upstream and downstream submodule dependencies explicitly. Changes to integration seams MUST be contract-tested.
...
## Article VI: HelixAgent-Specific Mandates

- §6.1: Container orchestration is owned EXCLUSIVELY by the HelixAgent binary (make build -> ./bin/helixagent).
- §6.2: All 51+ LLM providers MUST have dedicated implementations in internal/llm/providers/.
- §6.3: The 8-phase debate protocol MUST be implemented with full topology support.
- §6.4: MCP adapters MUST support 45+ implementations with containerized MCP servers.
- §6.5: Remote distribution hosts MUST be registered dynamically via Containers/.env — no hardcoded hosts.
...
Any submodule that imports HelixAgent MUST inherit HelixAgent's testing and anti-bluff requirements for all integration points.
...
*End of Constitution of HelixAgent*
