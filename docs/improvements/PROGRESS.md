...
- [~] P1.5-WP1-T01.01 — recursive fetch + pull every submodule **HALTED** (init aborted at helix_agent/DebateOrchestrator unreachable; full log in `docs/improvements/p1-5-fetch.log`)
...
- [x] P0-05 — migrate API keys from ../helix_agent/.env  ← (this commit)
- [x] P0-06 — update .gitignore (root + inner)  ← (this commit)
- [x] P0-07 — refresh helix_code/helix_code/.env.example  ← (this commit)
...
- [x] P0-10 — create helix_code/{CLAUDE,AGENTS,CONSTITUTION}.md (inner Go-app governance triplet) ← (this commit)
...
## P2-F23 task list (Cline Browser Tool) — ALL CLOSED
- [x] P2-F23-T01 — bootstrap F23 evidence section + advance PROGRESS to F23  ← commit `64e499b`
- [x] P2-F23-T02 — browser/types.go + options.go: Snapshot + ScreenshotResult + ManagerStatus + sentinels + EnvVarHeadedMode + MaxSnapshotBytes + MaxScreenshotBytes (TDD)  ← commit `cdb323e`
- [x] P2-F23-T03 — browser/manager.go + session.go: BrowserManager atomic-pointer lifecycle + BrowserSession sync.Once close + sessionFactory seam (TDD)  ← commit `e0ff1bf`
- [x] P2-F23-T04 — browser_navigate_v2.go: browser_navigate Tool impl with WaitReady + lazy session-create + 30s timeout (TDD); legacy tools renamed to browser_legacy_*  ← commit `2bcb281`
- [x] P2-F23-T05 — browser_snapshot_v2.go: browser_snapshot Tool impl with html/text mode + 64KB cap + Truncated flag (TDD)  ← commit `ec0b3cc`
- [x] P2-F23-T06 — browser_click_type_v2.go: browser_click + browser_type Tool impls with NodeVisible + 5s + ClickWait settle (TDD)  ← commit `659307d`
- [x] P2-F23-T07 — browser_screenshot_v2.go: PNG-magic + DecodeConfig + size>1024 + tempdir 0600 (TDD)  ← commit `3ca50a3`
...
## P2-F24 task list (Codex Project Memory) — ALL CLOSED
- [x] P2-F24-T01 — bootstrap F24 evidence section + advance PROGRESS to F24
- [x] P2-F24-T02 — projectmemory/types.go: Memory + Render + sentinels + MaxMemoryBytes + DiscoveryFilenames (TDD)
- [x] P2-F24-T03 — projectmemory/loader.go: parent-walk + git-root-stop + user overlay + truncation (TDD with real tempdirs)
- [x] P2-F24-T04 — projectmemory/registry.go: atomic-pointer Snapshot/Set/Reload + MemorySnapshotter (TDD -race)
- [x] P2-F24-T05 — projectmemory/watcher.go: fsnotify + 200ms debounce + graceful degrade (TDD with real fsnotify)
...
- 2026-05-07 — Feature 24 (Codex Project Memory) closed. 8 task commits (T01 `f55b3e3`, T02 `fd90eed`, T03 `99d3971`, T04 `0760562`, T05 `d740964`, T06 `7af2859`, T07 `40927fc`, T08 close-out — this commit). Real, end-to-end project memory subsystem modelled on codex's AGENTS.md pattern. NEW `internal/projectmemory/` package: `Memory` (immutable value with Render() = Project + delimiter + User) + `MemoryLoader` (parent-walk discovery for `helixcode.md` / `codex.md` / `AGENTS.md`, case-insensitive two-pass scan, stops at git-root marker or filesystem root, real tempdir-tested) + `MemoryRegistry` (atomic.Pointer[Memory] Snapshot lock-free / Set / Reload mu-serialised + MemorySnapshotter interface) + `MemoryWatcher` (real fsnotify v1.9.0 + 200 ms debounce + parent-dir watch survives atomic-write rename + graceful degrade on watch failure). NEW `/memory` slash (status/show/edit/reload) with `$EDITOR` fallback to `vi`. `BaseAgent.SetMemoryRegistry` + `getSystemPrompt` prepends `Memory.Render()` per-call (lock-free Snapshot — NOT cached at construct time; spec §5.2 Bluff #3). User overlay at `$XDG_CONFIG_HOME/helixcode/memory.md` loaded AFTER project memory (security invariant). 64 KB cap with positive `TruncatedProject` / `TruncatedUser` flags. Composes with F15 subagents (shared registry per CLI-instance), F04 worktrees (per-cwd discovery), F21 approval / F22 autocommit / F23 browser (orthogonal — registry is pre-existing main.go state). Zero new external deps (`fsnotify v1.9.0` already direct in `helix_code/go.mod`; go.mod + go.sum diff-clean post `go mod tidy`). Five-phase Challenge harness PASS (PHASE-A PROJECT-ONLY 4/4 + PHASE-B MISSING-FILE-GRACEFUL 4/4 + PHASE-C HOT-RELOAD 3/3 + PHASE-D PROJECT-PLUS-USER 3/3 + PHASE-E TRUNCATION 3/3 = 17/17 checks) with positive runtime evidence per Article XI §11.9 against real tempdirs + real fsnotify; cross-compile linux/amd64 PASS (45 MB binary); anti-bluff smoke clean across all F24 surface (HelixCode + Challenges); unit/integration tests all green under -race. **Fourth Phase 2 feature shipped.** F25 next candidate per synthesis design §4.2.
- 2026-05-07 — Feature 23 (Cline Browser Tool) closed. 10 task commits (T01 `64e499b`, T02 `cdb323e`, T03 `e0ff1bf`, T04 `2bcb281`, T05 `ec0b3cc`, T06 `659307d`, T07 `3ca50a3`, T08 `ad0c0df`, T09 `f39f686`, T10 close-out — this commit). Real, end-to-end 6-tool browser-automation suite modelled on cline: browser_navigate / browser_snapshot / browser_click / browser_type / browser_screenshot / browser_close. Atomic-pointer BrowserManager; lazy-create on first navigate; sync.Once close. Headless default; HELIXCODE_BROWSER_HEADED=true opt-in. Screenshots written to $XDG_DATA_HOME/helixcode/browser/screenshots/<session>/<n>.png with PNG-magic + DecodeConfig + size>1024 verification. F21 RequiresApproval per-tool: navigate/click/type/close → LevelEdit; snapshot/screenshot → LevelReadOnly. /browser slash (status/navigate/close). NO cobra. Coexists with renamed legacy browser_legacy_* multi-browser tools. Composes with F21 approval (gates LevelEdit tools), F22 autocommit (no spurious commits — browser tools have no path param), F13 LSP trigger (no-op for browser tools), F04 worktree (browser per CLI-instance). Zero new external deps (chromedp v0.15.1 + cdproto already direct). Seven-phase Challenge harness PASS (PHASE-A NAVIGATE-AND-SNAPSHOT 4/4 + PHASE-B SNAPSHOT-MODE-TEXT 2/2 + PHASE-C CLICK-MUTATES-DOM 3/3 + PHASE-D TYPE-INTO-INPUT 2/2 + PHASE-E SCREENSHOT-PNG-MAGIC 4/4 + PHASE-F CLOSE-TEARS-DOWN 2/2 + PHASE-G CONCURRENT-SESSION-SHARING 2/2) with positive runtime evidence per Article XI §11.9 against real chromium subprocess; cross-compile linux/amd64 PASS (45 MB binary); anti-bluff smoke clean across all F23 surface (HelixCode + Challenges); 7/7 integration tests PASS (3.737s) against real chromium. **Third Phase 2 feature shipped.** F24 next candidate per synthesis design §4.2.
...
- **Historical SSH key leak (remediated in P0-T08.5):** `id_rsa` + `id_rsa.pub` at `helix_code/test/workers/ssh_keys/` were committed as test fixtures before this programme. Their material lives in git history forever and is considered compromised. Mitigation: keys were ephemerally test-only (no production trust), replaced with auto-generated ed25519 ephemeral keys via `helix_code/test/workers/ssh_keys/generate-test-keys.sh`, removed from the index via `git rm --cached`. Any future production system that erroneously trusts the leaked public key must reject it.
...
- **HelixAgent stale cli_agents pins (discovered during P0-03):** 13 of 60 cli_agents under `helix_agent/cli_agents/` cannot be initialized because HelixAgent's recorded submodule SHAs no longer exist on the corresponding upstream remotes. Affected: `aider, conduit, continue, HelixCode, kilo-code, kiro-cli, mobile-agent, ollama-code, opencode-cli, openhands, plandex, roo-code, superset`. Each Phase 2 sub-spec for the affected agent must first bump HelixAgent's pointer (commit IN HelixAgent itself, then bump HelixAgent's pointer in this meta-repo) to a SHA that exists upstream. Phase 1 priority `claude-code` is NOT affected — fully populated. Per spec §1.3 N2, HelixAgent rewrite is out of scope for this programme; the per-agent pin bumps go through HelixAgent's own governance.
- **SonarQube + Snyk live-scan deferral (P0-T08.7):** The scan infrastructure (compose files, scripts, BootManager binary, Challenges) is fully wired and configuration-validated. Live scans CANNOT run until the user rotates the leaked credentials from `helix.security.json` (remediated in P0-T08.5 but historical values are compromised). Action required: (1) generate new SonarQube API token, (2) set `SONAR_TOKEN` + `SONARQUBE_PROJECT_KEY` + `SONARQUBE_PROJECT_NAME` in `helix_code/.env`, (3) generate new Snyk token, (4) set `SNYK_TOKEN` in `helix_code/.env`, (5) run `make scan-sonarqube` / `make scan-snyk`. This is NOT a code defect — it is a security-rotation dependency on the operator.
- **LLMsVerifier dual-pin divergence (discovered during P0-04):** RESOLVED in P1.5-WP2; doc previously stale. The transitive duplicate `helix_agent/LLMsVerifier` was eliminated in WP2 (only a backup remains at `helix_agent/.container-caps-backup-20260425-151947/LLMsVerifier`). `helix_agent/go.mod:232` now replaces `digital.vasic.llmsverifier => ../submodules/llms_verifier/llm-verifier`, so HelixAgent transitively consumes the canonical pin. The pin-parity gate `scripts/verify-llmsverifier-pin-parity.sh` is degenerate (regression-tripwire only; header confirms WP2 elimination). `make verify-foundation` evidence (close-out⁴⁵, 2026-05-15): exits 0, "0 failures, all gates passed" — no `VERIFY_FOUNDATION_WARN_ONLY` waiver required. Original divergence text preserved verbatim above this annotation in the historical evidence at `05_phase_0_evidence.md` §P0-04.
...
- **LLMsVerifier build** — Fixed go.mod replace path `../../Challenges` → `../../../Challenges` (LLMsVerifier at `submodules/llms_verifier/`, Challenges at meta-repo root `challenges/`). `go build ./...` and `go mod tidy` both pass clean.
- **HelixQA build** — Fixed 4 replace directives that pointed to wrong paths: `../LLMsVerifier/llm-verifier` → `../submodules/llms_verifier/llm-verifier`, `../DocProcessor` → `../submodules/doc_processor`, `../LLMOrchestrator` → `../submodules/llm_orchestrator`, `../VisionEngine` → `../submodules/vision_engine`. `go build ./...` and `go mod tidy` both pass clean.
...
- `helixcode wizard` — confirmed at `helix_code/cmd/cli/wizard_cmd.go:83`
- `/approval`, `/memory`, `/lsp`, `/theme`, `/telemetry`, `/worktree` — all present under `helix_code/internal/commands/`
- `cli --list-models` and interactive `> models` — confirmed at `helix_code/cmd/cli/main.go:347, 1681`
- `cli sessions list` — confirmed at `helix_code/cmd/cli/main.go:1822`
- `cli lsp list-servers` — confirmed at `helix_code/cmd/cli/main.go:1850`
