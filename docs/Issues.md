...
**Progress (2026-05-29):** Static audit ledger produced (`docs/qa/HXC-029/compliance-ledger.md`). Both CONFIRMED code-level violations FIXED: (1) `helix_code/tests/regression/server_timeout_test.go` `TestServerStability` was `t.Skip("run manually")` over an unimplemented body (a §11.4 PASS-bluff) — now a real self-driving test that stands up a `net/http` server with the config's timeout semantics and asserts idle-survival with real HTTP I/O; PASS `-count=3` (~1.75s each); also fixed the sibling `TestServerTimeoutConfiguration` CWD-dependent hard-fail (now skips-with-reason); whole `tests/regression` package green. (2) `helix_code/tests/e2e/scripts/clean.sh:59` interactive `read -p` → now `--force`/`CLEAN_FORCE=1`/TTY-gated, keeps results non-interactively (no hang); `bash -n` clean. **Bank verification (2026-05-29):** 7 banks now self-driving + anti-bluff-verified against the live `:8080` server (3 deterministic runs + flip-expectation mutation-proof each, evidence under `docs/qa/HXC-029/`): 4 API banks (`helixcode-full-qa-api`, `entity-management`, `admin-operations`, `security-validation`) + 3 CLI banks (`cli-agents-comprehensive` 6 real PASS, `aichat-bash-tools-comprehensive` 9 PASS, `cli-agents-test-helixagent` 7 PASS) driving the real `helix_code/bin/cli` via `os/exec` with asserted exit codes/output. `grep -c manual-review-required`=0 on all 7. Honest `_skip` for absent tools (`aider` REPL/`:7061` ensemble down, `aichat`/`openhands` not on PATH, Go-API internals with no CLI surface) — no fabricated PASS (§11.4.98). helix_qa HEAD `14be9eb`. **Integration long-tail (2026-05-29):** RESOLVED — classified all 31 `helix_code/tests/integration/**` files: **31/31 are already self-driving** (0 require human action during execution; gated only by build-tags / `testing.Short` / infra-presence), 0 needed conversion, 0 obsolete. Real captured evidence (`docs/qa/HXC-029/integration-classification/`): `go test -tags=integration ./tests/integration/` → 110 PASS / 20 SKIP (honest infra/cred-unavailable) / 3 FAIL. The 3 FAILs are NOT manual deps — they are a real i18n-render regression the self-driving tests correctly catch (filed as **HXC-036**). REMAINING: Playwright/Android banks only (need real browser/emulator drivers).
...
## HXC-036 — Systemic CONST-046 i18n defect: 74 packages emit raw message-ID keys to users because boot-time translator wiring was never implemented

**Status:** Open
**Type:** Bug
**Severity:** High (every CONST-046-migrated user-facing CLI string renders as a raw key like `askuser_prompt_invalid_choice_hint` instead of the resolved+interpolated text — a §11.9 "tests-green-but-feature-broken-for-users" defect class)
**Discovered:** 2026-05-29 (surfaced by the HXC-029 §11.4.98(F) integration-classification: 3 self-driving tests `TestAskUser_InvalidInputThenValid` / `TestAskUser_PreviewVisibleInOutput` / `TestApproval_AutoEdit_PromptsRun_UserAllows` correctly FAIL because the prompter output contains raw keys, not "1-3")
**Discovered-By:** AI
**Forensic-anchor:** §11.9 / CONST-046 — user-facing text must be resolved at runtime, not shown as a literal key.
**Root cause (CONFIRMED via §11.4.102 systematic-debugging, module-wide grep evidence):** the CONST-046 i18n migration (rounds 93–455) created **74** per-package `SetTranslator(...)` DI seams, per-package `i18n/` subpackages (`Translator` interface + `NoopTranslator{}` loud-echo default), and `active.en.yaml` bundles — but the documented "consuming binary builds an `*i18n.Localizer` from the bundle, wraps it in `*i18nadapter.Translator`, and calls `<pkg>.SetTranslator(...)` at boot" step was **NEVER WRITTEN**: `grep -rn '\.SetTranslator(' --include='*.go'` (non-test) across the whole module returns **0 call sites**; `i18nadapter` is never constructed at boot; the bundles are not even `go:embed`'d. So every one of the 74 packages runs with `NoopTranslator{}` → `tr()` returns the message ID verbatim → users see raw keys. Unit tests passed because they assert the NoopTranslator echo; the defect only shows in end-user-facing rendering (exactly the §11.9 scenario).
**Reproduction:** `cd helix_code && go test -tags=integration -run 'TestAskUser_InvalidInputThenValid' ./tests/integration/...` → FAIL ("does not contain 1-3"; output shows `askuser_prompt_invalid_choice_hint`). Evidence: `docs/qa/HXC-029/integration-classification/i18n-regression-evidence.txt`.
**Fix direction (design fork — needs decision):** (A) implement boot-time wiring — a central helper that, per package, loads its `active.en.yaml` (add `go:embed`), builds an `*i18n.Localizer`, wraps via `i18nadapter.New`, and calls `<pkg>.SetTranslator`, invoked from every entry point (cmd/server, cmd/cli, cmd/infrastructure, applications/*); the integration tests must exercise that wiring; OR (B) change each package's default from `NoopTranslator{}` to a self-initializing real translator (lazy-load its own embedded bundle), keeping NoopTranslator only for explicit unit-test opt-in — more robust (impossible to forget) but departs from the documented inject-at-boot design. Either way: per-package resolved-text tests + the 3 failing integration tests must pass.
**Closure criteria:** the 3 failing integration tests PASS with resolved interpolated text; a representative sample (≥askuser + approval + 3 others) proven to render real text not keys via captured `go test` evidence; anti-bluff per §11.4/§11.9; no NoopTranslator in production render paths.
**Composes-with:** §11.9, CONST-046, §11.4.98 (caught by the live-test full-automation sweep), §11.4.102 (systematic-debugging root-caused it), HXC-029.
