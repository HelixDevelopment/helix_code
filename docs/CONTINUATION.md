...
**Last updated: 2026-05-13T09:00:00Z (mistborn.local distribution + helix-bridge + helix-qa anti-bluff harness expanded 6 → 27 checks; 5 real production bugs surfaced and fixed at root cause this session: (1) BLUFF-002 stub in `getLLMProvider` (unknown id now 404 not fabricated "available"), (2) `task_data NOT NULL` constraint violation on POST /tasks with empty parameters (now defaults to empty map), (3) `tasks: null` vs `tasks: []` JSON contract bluff (now array), (4) GET /projects 401 with valid JWT due to context-key mismatch — middleware sets "user" as *auth.User, handler looked up nonexistent "user_id" string (now uses established convention), (5) `projects: null` vs `projects: []` (now array). New unit tests: TestGetLLMProvider_UnknownIDReturns404. JWT/session tokens redacted in QA evidence per CONST-042 defense-in-depth. All 4 meta-repo remotes at `c332f98`. run_all_challenges 12/12 PASS. helix-qa 27/27 PASS. helix-bridge: 3/5 LLM providers respond live.)
