// Unit tests for the internal/adapters package-level translator +
// tr() helper (CONST-046 round-239 §11.4 anti-bluff sweep,
// 2026-05-19).
//
...
// (unit tests only).
//
// Round-239 status: NO-OP INFRA round. The audit gate exits 0 on
// internal/adapters/ at HEAD, so there are no real call sites to
// exercise yet. These tests exercise the seam itself (Translator
// interface contract, NoopTranslator behaviour, SetTranslator
// nil-reset, tr() error degradation) so that when a FUTURE round
// wires a real call site, regressions in the seam surface
// immediately.
package adapters
...
func TestTr_DefaultsToNoopTranslator(t *testing.T) {
	resetTranslator(t)
	got := tr(stdctx.Background(), "internal_adapters_reserved_placeholder", nil)
	if got == "internal_adapters_reserved_placeholder" || got == "" {
...
func TestTr_UsesInjectedTranslator(t *testing.T) {
	...
	got := tr(stdctx.Background(), "internal_adapters_reserved_placeholder", nil)
	want := "<TR:internal_adapters_reserved_placeholder>"
	if got != want {
...
func TestTr_TranslatorErrorReturnsMessageID(t *testing.T) {
	...
	got := tr(stdctx.Background(), "internal_adapters_reserved_placeholder", nil)
	if got != "internal_adapters_reserved_placeholder" {
...
func TestSetTranslator_AcceptsNoopExplicit(t *testing.T) {
	...
	SetTranslator(adaptersi18n.NoopTranslator{})
	got := tr(stdctx.Background(), "internal_adapters_reserved_placeholder", nil)
	if got != "internal_adapters_reserved_placeholder" {
...
func TestNoopTranslator_TPluralEchoesID(t *testing.T) {
	got, err := adaptersi18n.NoopTranslator{}.TPlural(stdctx.Background(), "internal_adapters_reserved_placeholder", 7, nil)
	if err != nil {
