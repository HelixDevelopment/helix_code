// bundle.go — boot-time Translator constructor for internal/workflow
// (HXC-036 Phase 4, 2026-05-29).
//
// internal/workflow/i18n is the SHARED CONST-046 i18n package for both
// internal/workflow/autonomy and internal/workflow/planmode — their
// per-package SetTranslator seams (i18n_seam.go) both accept this
// package's Translator type. The CONST-046 migration (rounds 185/424/457)
// authored the Translator interface + NoopTranslator{} loud-echo default
// + bundles/active.en.yaml catalogue, but the boot-time wiring that builds
// a REAL translator from the shipped bundle was never written — every
// caller ran with NoopTranslator{}, so users saw raw message-ID keys
// (e.g. "internal_workflow_mode_full_auto_label") instead of resolved +
// interpolated text. NewTranslator closes that gap: it embeds the
// active.en.yaml bundle, loads it into a pkg/i18n *Bundle, constructs an
// *i18n.Localizer, and wraps it in an *i18nadapter.Translator that
// structurally satisfies this package's Translator interface.
//
...
// injects the result via autonomy.SetTranslator + planmode.SetTranslator
// (both packages share this i18n package's Translator type).
//
...
func NewTranslator(langs ...string) (Translator, error) {
	...
	if err := bundle.LoadMessageFileFS(activeBundleFS, activeBundlePath); err != nil {
		return nil, fmt.Errorf("workflow i18n: load embedded %q: %w", activeBundlePath, err)
	}
	if len(langs) == 0 {
