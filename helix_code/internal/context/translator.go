// translator.go — CONST-046 message-ID resolver seam for internal/context.
//
// Round-151 §11.4 anti-bluff sweep (2026-05-18). Mirrors the
// "consumer defines its own Translator + tr() helper" pattern used
// by every other CONST-046-migrated package in this codebase
// (rounds 93/94/95/96/108/131/134/136-150).
package context
...
import (
	...
	"dev.helix.code/internal/context/i18n"
)
...
// migration minimally invasive — ContextManager methods are called
// from many sites and the global-manager free functions are
...
func init() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr,
			"internal/context i18n: embedded-bundle translator load failed; "+
