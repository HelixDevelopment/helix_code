// translator.go — CONST-046 message-ID resolver seam for internal/roocode.
//
// Round-236 §11.4 anti-bluff sweep (2026-05-19). Mirrors the
// "consumer defines its own Translator + tr() helper" pattern used
// by every other CONST-046-migrated package in this codebase
// (internal/approval, internal/approvalwire, internal/plantree,
// internal/projectmemory, internal/voice, …).
package roocode
...
import (
	...
	"dev.helix.code/internal/roocode/i18n"
)
...
// migration minimally invasive — the Roo-code surface (tool
// invocations, code generation, conversation store) reads from this
// pointer on every user-visible string emission; threading a
// translator through every constructor would inflate every Roo-code
...
func init() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr,
			"internal/roocode i18n: embedded-bundle translator load failed; "+
