// Unit tests for the internal/agent Translator interface +
...
func TestNoopTranslator_T_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.T(context.Background(), "internal_agent_task_cannot_be_nil", nil)
	if err != nil {
...
func TestNoopTranslator_TPlural_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.TPlural(context.Background(), "internal_agent_no_available_agent_found", 3, nil)
	if err != nil {
		t.Fatalf("NoopTranslator.TPlural returned error: %v", err)
	}
	if got != "internal_agent_no_available_agent_found" {
...
func TestNoopTranslator_T_IgnoresTemplateData(t *testing.T) {
	...
	// interpolated). If the NoopTranslator implementation ever silently
	// interpolated and returned a non-ID string, the assert below would
	// catch it.
	tr := NoopTranslator{}
	got, err := tr.T(context.Background(), "internal_agent_failed_shutdown_agent", map[string]any{"AgentID": "a1", "Err": "boom"})
	if err != nil {
		t.Fatalf("NoopTranslator.T returned error: %v", err)
	}
	if got != "internal_agent_failed_shutdown_agent" {
...
func TestFakeTranslator_T_WrapsID(t *testing.T) {
	tr := fakeTranslator{}
	got, err := tr.T(context.Background(), "internal_agent_task_not_found", nil)
	if err != nil {
		t.Fatalf("fakeTranslator.T returned error: %v", err)
	}
	want := "<TRANSLATED:internal_agent_task_not_found>"
	if got != want {
...
func TestFakeTranslator_T_FailOnID(t *testing.T) {
	tr := fakeTranslator{failOnID: "internal_agent_task_cannot_be_nil"}
	if _, err := tr.T(context.Background(), "internal_agent_task_cannot_be_nil", nil); err == nil {
		t.Fatal("fakeTranslator.T expected to fail for matching ID, got nil error")
	}
	// Non-matching IDs should still succeed.
	if _, err := tr.T(context.Background(), "internal_agent_task_not_found", nil); err != nil {
