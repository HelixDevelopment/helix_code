// Package i18n declares internal/agent's hardcoded-content abstraction
// per CONST-046 (round-147 §11.4 anti-bluff sweep, 2026-05-18).
//
// internal/agent hosts the BaseAgent + Coordinator + Workflow surface
// — every agent-orchestration error message emitted from this package
// reaches end users through CLI output, API responses, and TaskResult
...
// pattern established by rounds 93/94/95/96/108/131/134/136-145
...
// performance_optimization / security_fix_standalone) and round-146
// (internal/auth) for three reasons: (1) uniform pattern across the
...
// pkg/i18n's bundle-loading, (3) internal/agent could plausibly be
...
// internal/agent/i18n/bundles), wraps it in *i18nadapter.Translator,
// and stores it via agent.SetTranslator. The package-level tr()
...
// Translator is the contract internal/agent uses for every
