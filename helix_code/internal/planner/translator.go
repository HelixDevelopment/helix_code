// translator.go — CONST-046 message-ID resolver seam for internal/planner.
//
// Round-232 §11.4 anti-bluff sweep (2026-05-19). Mirrors the
// "consumer defines its own Translator + tr() helper" pattern used
// by every other CONST-046-migrated package in this codebase
// (most recently rounds 221..226).
package planner
...
import (
	...
	"dev.helix.code/internal/planner/i18n"
)
...
// migration minimally invasive — the planner executor runs in tight
// loops over plan steps and tool Description()/Validate() are called
// without a context parameter, so threading a translator through
// every call would inflate the API surface; the package-level seam
// keeps it behind a single pointer fetch.
//
// HXC-014b §11.4.85: both reads (tr) and writes (SetTranslator) of
// this package-level seam MUST be guarded by translatorMu — the
// executor runs strings through tight loops while SetTranslator may
// be re-invoked at boot/reconfiguration, so an unguarded access is a
...
func init() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr,
			"internal/planner i18n: embedded-bundle translator load failed; "+
