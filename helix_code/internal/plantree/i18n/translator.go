// Package i18n declares internal/plantree's hardcoded-content
// abstraction per CONST-046 (round-233 §11.4 anti-bluff sweep,
// 2026-05-19).
//
// internal/plantree hosts the plan-tree tool surface — six
// tools.Tool implementations (PlanCreateTool, PlanBranchTool,
// PlanMergeTool, PlanListTool, PlanShowTool, PlanDeleteTool)
// whose Description() returns are surfaced to operators via CLI
// help text + tool-registry surfaces. Every literal description
// is therefore a CONST-046 candidate.
//
// We follow the "consumer defines its own Translator interface"
// pattern established by rounds 93/94/95/96/108/131/134/136-232
// (most recently internal/voice 226, internal/secrets 225,
// internal/render 224, internal/quality 223) for three reasons:
// (1) uniform pattern across the codebase keeps mental overhead
// low, (2) the Translator interface is the natural seam for
// stubbing in tests without dragging in pkg/i18n's bundle-loading,
// (3) internal/plantree could plausibly be extracted into its own
...
// internal/plantree/i18n/bundles), wraps it in *i18nadapter.Translator,
// and stores it via plantree.SetTranslator. The package-level tr()
...
// Translator is the contract internal/plantree uses for every
