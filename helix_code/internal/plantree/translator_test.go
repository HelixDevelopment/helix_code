...
// wiring in internal/plantree (round-233 §11.4 anti-bluff sweep,
...
func TestSetTranslator_Nil_ResetsToNoop(t *testing.T) {
	...
	got := tr(context.Background(), "internal_plantree_tool_create_description", nil)
	if got != "<SENT:internal_plantree_tool_create_description>" {
		t.Fatalf("expected sentinel-wrapped output, got %q", got)
	}
	SetTranslator(nil)
	got = tr(context.Background(), "internal_plantree_tool_create_description", nil)
	if got == "internal_plantree_tool_create_description" || got == "" {
...
func TestTr_FallsBackToMessageIDOnError(t *testing.T) {
	...
	got := tr(context.Background(), "internal_plantree_tool_create_description", nil)
	if got != "internal_plantree_tool_create_description" {
...
// TestPlanCreateTool_Description_RoutesThroughTranslator is the
...
func TestPlanCreateTool_Description_RoutesThroughTranslator(t *testing.T) {
	resetTranslator(t)
	SetTranslator(sentinelTranslator{})

	tool := NewPlanCreateTool(nil)
	got := tool.Description()
	want := "<SENT:internal_plantree_tool_create_description>"
	if got != want {
		t.Fatalf("PlanCreateTool.Description did not route through translator: got %q, want %q", got, want)
	}
}

func TestPlanBranchTool_Description_RoutesThroughTranslator(t *testing.T) {
	resetTranslator(t)
	SetTranslator(sentinelTranslator{})

	tool := NewPlanBranchTool(nil)
	got := tool.Description()
	want := "<SENT:internal_plantree_tool_branch_description>"
	if got != want {
		t.Fatalf("PlanBranchTool.Description did not route through translator: got %q, want %q", got, want)
	}
}

func TestPlanMergeTool_Description_RoutesThroughTranslator(t *testing.T) {
	resetTranslator(t)
	SetTranslator(sentinelTranslator{})

	tool := NewPlanMergeTool(nil)
	got := tool.Description()
	want := "<SENT:internal_plantree_tool_merge_description>"
	if got != want {
		t.Fatalf("PlanMergeTool.Description did not route through translator: got %q, want %q", got, want)
	}
}

func TestPlanListTool_Description_RoutesThroughTranslator(t *testing.T) {
	resetTranslator(t)
	SetTranslator(sentinelTranslator{})

	tool := NewPlanListTool(nil)
	got := tool.Description()
	want := "<SENT:internal_plantree_tool_list_description>"
	if got != want {
		t.Fatalf("PlanListTool.Description did not route through translator: got %q, want %q", got, want)
	}
}

func TestPlanShowTool_Description_RoutesThroughTranslator(t *testing.T) {
	resetTranslator(t)
	SetTranslator(sentinelTranslator{})

	tool := NewPlanShowTool(nil)
	got := tool.Description()
	want := "<SENT:internal_plantree_tool_show_description>"
	if got != want {
		t.Fatalf("PlanShowTool.Description did not route through translator: got %q, want %q", got, want)
	}
}

func TestPlanDeleteTool_Description_RoutesThroughTranslator(t *testing.T) {
	resetTranslator(t)
	SetTranslator(sentinelTranslator{})

	tool := NewPlanDeleteTool(nil)
	got := tool.Description()
	want := "<SENT:internal_plantree_tool_delete_description>"
	if got != want {
		t.Fatalf("PlanDeleteTool.Description did not route through translator: got %q, want %q", got, want)
	}
}
...
// plan_tools.go appears in migratedMessageIDs(). If a future round
...
func TestNoopTranslator_Loud_Echo_AllMigratedIDs(t *testing.T) {
	noop := plantreei18n.NoopTranslator{}
...
func migratedMessageIDs() []string {
	// Round-233 migrated set. Keep alphabetical for easy diffing on
	// future rounds.
	return []string{
		"internal_plantree_tool_branch_description",
		"internal_plantree_tool_create_description",
		"internal_plantree_tool_delete_description",
		"internal_plantree_tool_list_description",
		"internal_plantree_tool_merge_description",
		"internal_plantree_tool_show_description",
	}
}
