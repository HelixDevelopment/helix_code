// translator.go — CONST-046 message-ID resolver seam for internal/kilocode.
//
// Round-231 §11.4 anti-bluff sweep (2026-05-19). Mirrors the
...
// migration minimally invasive — kilocode tools surface
// Description() + Schema() in hot registration paths and threading a
// struct-scoped translator through every constructor would inflate
...
var (
	...
	// loads, or kilocodei18n.NoopTranslator{} (loud message-ID echo) if it
	// does not. SetTranslator(nil) restores THIS — "nil = restore default"
	// means "restore correct prose", not "revert to raw-key echo".
	defaultTranslator kilocodei18n.Translator = kilocodei18n.NoopTranslator{}
	translator        kilocodei18n.Translator = kilocodei18n.NoopTranslator{}
)
...
func init() {
	tr, err := kilocodei18n.NewTranslator()
	if err != nil {
		fmt.Fprintf(os.Stderr,
			"internal/kilocode i18n: embedded-bundle translator load failed; "+
...
func SetTranslator(tr kilocodei18n.Translator) {
	translatorMu.Lock()
...
func tr(ctx context.Context, msgID string, data map[string]any) (result string) {
	...
	if active == nil {
		active = kilocodei18n.NoopTranslator{}
	}
