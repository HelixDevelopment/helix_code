// Package i18n declares internal/redis's hardcoded-content abstraction
// per CONST-046 (round-173 §11.4 anti-bluff sweep, 2026-05-19).
//
// internal/redis wraps the go-redis/v9 client and is consumed by
// every Helix component that talks to Redis (session store, queue,
// pubsub bus, rate-limiter, lock manager, etc.). Its connection
// and disabled-state error messages surface through API responses,
// CLI output, and structured logs — so every literal error string
// is a CONST-046 candidate. We follow the "consumer defines its own
// Translator interface" pattern established by rounds 93/94/95/96/
// 108/131/134/136-172 (Lazy / SelfImprove / HelixLLM / harmony_os /
...
// memory / notification / performance) for three reasons:
...
// (3) internal/redis could plausibly be extracted into its own
...
// internal/redis/i18n/bundles), wraps it in *i18nadapter.Translator,
// and stores it via redis.SetTranslator. The package-level tr()
...
// Translator is the contract internal/redis uses for every
