// Package i18n declares internal/auth's hardcoded-content abstraction
// per CONST-046 (round-146 §11.4 anti-bluff sweep, 2026-05-18).
//
// internal/auth is the central authentication/authorization package
// used by every server, CLI, and application path. Its error
// messages reach end users through API responses, CLI output, and
// UI error banners — so every literal error string is a CONST-046
// candidate. We follow the "consumer defines its own Translator
// interface" pattern established by rounds 93/94/95/96/108/131/134/
// 136-145 (Lazy / SelfImprove / HelixLLM / harmony_os / helix_config
// / helixllm / cli / server / desktop / terminal_ui / ios / android
// / aurora_os / config_test / security_test / security_fix /
// performance_optimization / security_fix_standalone) for three
...
// bundle-loading, (3) internal/auth could plausibly be extracted
// into its own submodule for reuse by other Helix services without
...
// internal/auth/i18n/bundles), wraps it in *i18nadapter.Translator,
// and stores it via auth.SetTranslator. The package-level tr()
...
// Translator is the contract internal/auth uses for every
