// translator.go — CONST-046 message-ID resolver seam for internal/auth.
//
// Round-146 §11.4 anti-bluff sweep (2026-05-18). Mirrors the
// "consumer defines its own Translator + tr() helper" pattern used
// by every other CONST-046-migrated package in this codebase
// (rounds 93/94/95/96/108/131/134/136-145).
package auth
...
import (
	...
	"dev.helix.code/internal/auth/i18n"
)
...
// migration minimally invasive — every existing AuthService method
// already takes its own context, so threading a struct-scoped
...
func init() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr,
			"internal/auth i18n: embedded-bundle translator load failed; "+
