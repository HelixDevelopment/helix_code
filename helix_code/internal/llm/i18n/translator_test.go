// Unit tests for the internal/llm Translator interface + NoopTranslator
...
func TestNoopTranslator_T_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.T(context.Background(), "internal_llm_wizard_anthropic_apikey_required", nil)
	if err != nil {
		t.Fatalf("NoopTranslator.T returned error: %v", err)
	}
	if got != "internal_llm_wizard_anthropic_apikey_required" {
...
func TestNoopTranslator_TPlural_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.TPlural(context.Background(), "internal_llm_wizard_bedrock_region_required", 1, nil)
	if err != nil {
		t.Fatalf("NoopTranslator.TPlural returned error: %v", err)
	}
	if got != "internal_llm_wizard_bedrock_region_required" {
...
func TestNoopTranslator_T_IgnoresTemplateData(t *testing.T) {
	...
	tr := NoopTranslator{}
	got, err := tr.T(context.Background(), "internal_llm_writer_nil_result", map[string]any{"Path": "/tmp/x"})
	if err != nil {
		t.Fatalf("NoopTranslator.T returned error: %v", err)
	}
	if got != "internal_llm_writer_nil_result" {
...
func TestFakeTranslator_T_WrapsID(t *testing.T) {
	tr := fakeTranslator{}
	got, err := tr.T(context.Background(), "internal_llm_wizard_azure_endpoint_required", nil)
	if err != nil {
		t.Fatalf("fakeTranslator.T returned error: %v", err)
	}
	want := "<TRANSLATED:internal_llm_wizard_azure_endpoint_required>"
	if got != want {
...
func TestFakeTranslator_T_FailOnID(t *testing.T) {
	tr := fakeTranslator{failOnID: "internal_llm_writer_empty_path"}
	if _, err := tr.T(context.Background(), "internal_llm_writer_empty_path", nil); err == nil {
		t.Fatal("fakeTranslator.T expected to fail for matching ID, got nil error")
	}
	// Non-matching IDs should still succeed.
	if _, err := tr.T(context.Background(), "internal_llm_wizard_vertexai_location_required", nil); err != nil {
