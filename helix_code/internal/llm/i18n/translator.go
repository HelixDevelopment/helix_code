// Package i18n declares internal/llm's hardcoded-content abstraction
// per CONST-046 (round-161 §11.4 anti-bluff sweep, 2026-05-19).
//
// internal/llm is the orchestration surface for every LLM provider
// (Anthropic, Bedrock, Vertex AI, Azure, OpenAI, Ollama, …) plus
// the first-run cloud-provider setup wizard. Its validation
// messages and config-write failure messages reach end users
// through the TUI wizard, the CLI flags path, and the API error
// response surface — so every literal user-facing string is a
// CONST-046 candidate. We follow the "consumer defines its own
// Translator interface" pattern established by rounds 93/94/95/96
// /108/131/134/136-146 (Lazy / SelfImprove / HelixLLM / harmony_os
...
// / auth) for three reasons: (1) uniform pattern across the
// codebase keeps mental overhead low, (2) the Translator interface
// is the natural seam for stubbing in tests without dragging in
// pkg/i18n's bundle-loading, (3) internal/llm could plausibly be
// extracted into its own submodule for reuse by other Helix
// services without restructuring the i18n surface.
//
// The wire-in path at boot is: the consuming binary constructs an
// *i18n.Localizer (loaded with the active.en.yaml bundle from
// internal/llm/i18n/bundles), wraps it in *i18nadapter.Translator,
// and stores it via llm.SetTranslator. The package-level tr()
// helper resolves message IDs through this interface and falls
// back to NoopTranslator{} when no real translator has been wired
...
// Translator is the contract internal/llm uses for every
