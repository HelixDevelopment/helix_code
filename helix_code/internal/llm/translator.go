// translator.go — CONST-046 message-ID resolver seam for internal/llm.
//
// Round-161 §11.4 anti-bluff sweep (2026-05-19). Mirrors the
// "consumer defines its own Translator + tr() helper" pattern used
// by every other CONST-046-migrated package in this codebase
// (rounds 93/94/95/96/108/131/134/136-146).
package llm

import (
	"context"

	"dev.helix.code/internal/llm/i18n"
)
...
// migration minimally invasive — wizard.go's validateWizardForm is
// a pure-logic free function with no struct receiver to hang a
// translator field on, so threading a struct-scoped translator
