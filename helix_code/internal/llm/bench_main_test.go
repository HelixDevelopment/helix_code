...
// binary so that provider-construction logging (e.g. "OpenAI provider
// initialized with N models") does not interleave into and corrupt the
// `go test -bench` result lines. This keeps the captured baseline
...
// process concern only and the llm package had no prior TestMain.
