...
// migration minimally invasive — EventBus methods are called from
// many sites and threading a struct-scoped translator would inflate
...
var (
	...
	translator   eventi18n.Translator = eventi18n.NoopTranslator{}
)
...
func SetTranslator(tr eventi18n.Translator) {
	translatorMu.Lock()
	defer translatorMu.Unlock()
	if tr == nil {
		translator = eventi18n.NoopTranslator{}
		return
	}
	translator = tr
}
...
func tr(ctx context.Context, msgID string, data map[string]any) (result string) {
	...
	if active == nil {
		active = eventi18n.NoopTranslator{}
	}
