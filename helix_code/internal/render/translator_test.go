// Unit tests for the internal/render package-level translator +
// tr() helper (CONST-046 round-224 §11.4 anti-bluff sweep,
// 2026-05-19).
//
...
// (unit tests only).
//
// Round-224 status: NO-OP INFRA round. The audit gate reports zero
// CONST-046 hardcoded-content violations in internal/render/ at HEAD,
...
func TestTr_DefaultsToNoopTranslator(t *testing.T) {
	resetTranslator(t)
	got := tr(stdctx.Background(), "internal_render_reserved_placeholder", nil)
	if got == "internal_render_reserved_placeholder" || got == "" {
...
func TestTr_UsesInjectedTranslator(t *testing.T) {
	...
	got := tr(stdctx.Background(), "internal_render_reserved_placeholder", nil)
	want := "<TR:internal_render_reserved_placeholder>"
	if got != want {
...
func TestTr_TranslatorErrorReturnsMessageID(t *testing.T) {
	...
	got := tr(stdctx.Background(), "internal_render_reserved_placeholder", nil)
	if got != "internal_render_reserved_placeholder" {
...
func TestSetTranslator_AcceptsNoopExplicit(t *testing.T) {
	...
	SetTranslator(renderi18n.NoopTranslator{})
	got := tr(stdctx.Background(), "internal_render_reserved_placeholder", nil)
	if got != "internal_render_reserved_placeholder" {
...
func TestNoopTranslator_TPluralEchoesID(t *testing.T) {
	got, err := renderi18n.NoopTranslator{}.TPlural(stdctx.Background(), "internal_render_reserved_placeholder", 7, nil)
	if err != nil {
