// Package i18n declares internal/config's hardcoded-content
// abstraction per CONST-046 (round-150 §11.4 anti-bluff sweep,
// 2026-05-18). Mirrors the "consumer defines its own Translator
// interface" pattern of rounds 93..149 (most recently internal/auth
// 146, internal/agent 147, internal/cognee 148, internal/commands
// 149). Wire-in path at boot: consuming binary builds an
// *i18n.Localizer (loaded with active.en.yaml from
// internal/config/i18n/bundles), wraps it in *i18nadapter.Translator,
// stores via config.SetTranslator. The package-level tr() helper
...
// Translator is the contract internal/config uses for every
