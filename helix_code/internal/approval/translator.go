// translator.go — CONST-046 message-ID resolver seam for internal/approval.
//
// Round-221 §11.4 anti-bluff sweep (2026-05-19). Mirrors the
...
import (
	...
	"dev.helix.code/internal/approval/i18n"
)
...
// migration minimally invasive — the ApprovalManager has hot-path
// atomic reads in CheckApproval, so threading a translator through
// every call would inflate cost; the package-level seam keeps it
// behind a single pointer fetch.
//
...
func init() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr,
			"internal/approval i18n: embedded-bundle translator load failed; "+
