// bundle.go — boot-time Translator constructor for internal/approval
// (HXC-036 Phase 1, 2026-05-29).
//
// The CONST-046 round-221 migration created the Translator interface +
...
// saw raw message-ID keys (e.g. "internal_approval_prompt_allow_tool")
...
// injects the result via approval.SetTranslator.
//
...
func NewTranslator(langs ...string) (Translator, error) {
	...
	if err := bundle.LoadMessageFileFS(activeBundleFS, activeBundlePath); err != nil {
		return nil, fmt.Errorf("approval i18n: load embedded %q: %w", activeBundlePath, err)
	}
	if len(langs) == 0 {
