// bundle.go — boot-time Translator constructor for internal/clarification
...
func NewTranslator(langs ...string) (Translator, error) {
	...
	if err := bundle.LoadMessageFileFS(activeBundleFS, activeBundlePath); err != nil {
		return nil, fmt.Errorf("internal/clarification i18n: load embedded %q: %w", activeBundlePath, err)
	}
	if len(langs) == 0 {
