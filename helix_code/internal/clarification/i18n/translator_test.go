// Unit tests for the internal/clarification/i18n Translator +
// NoopTranslator (CONST-046 round-222 §11.4 anti-bluff sweep,
// 2026-05-19).
//
// Mocks ALLOWED per CONST-050(A) (unit tests only). These tests
// guard the interface contract + the Noop fallback so the
// consumer-side tr() helper has a stable foundation.
package i18n
...
func TestNoopTranslator_TReturnsIDUnchanged(t *testing.T) {
	n := NoopTranslator{}
	got, err := n.T(context.Background(), "internal_clarification_some_id", nil)
	if err != nil {
		t.Fatalf("NoopTranslator.T returned error %v, want nil", err)
	}
	if got != "internal_clarification_some_id" {
...
func TestNoopTranslator_TIgnoresTemplateData(t *testing.T) {
	...
	n := NoopTranslator{}
	got, _ := n.T(context.Background(), "internal_clarification_failed_x", map[string]any{"Prompt": "fix the bug"})
	if got != "internal_clarification_failed_x" {
...
func TestNoopTranslator_TPluralReturnsIDUnchanged(t *testing.T) {
	n := NoopTranslator{}
	got, err := n.TPlural(context.Background(), "internal_clarification_plural_id", 5, nil)
	if err != nil {
		t.Fatalf("NoopTranslator.TPlural returned error %v, want nil", err)
	}
	if got != "internal_clarification_plural_id" {
