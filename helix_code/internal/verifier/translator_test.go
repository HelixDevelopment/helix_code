// Unit tests for the internal/verifier package-level translator +
// tr() helper (CONST-046 round-182 §11.4 anti-bluff sweep,
// 2026-05-19).
//
...
// (unit tests only).
//
// Round-182 status: NO-OP INFRA round. The audit gate reports only
// two CONST-046 findings in internal/verifier/ at HEAD, both for the
// model DisplayName/Name string "Grok-3 Fast Beta" in
// fallback_models.go — model identity tokens out of CONST-046 scope
// per the hardware/logging precedent. There are therefore no real
// call sites to exercise yet. These tests exercise the seam itself
// (Translator interface contract, NoopTranslator behaviour,
// SetTranslator nil-reset, tr() error degradation) so that when a
// FUTURE round wires a real call site, regressions in the seam
// surface immediately.
package verifier
...
func TestTr_UsesInjectedTranslator(t *testing.T) {
	...
	got := tr(stdctx.Background(), "internal_verifier_reserved_placeholder", nil)
	want := "<TR:internal_verifier_reserved_placeholder>"
	if got != want {
...
func TestSetTranslator_AcceptsNoopExplicit(t *testing.T) {
	...
	SetTranslator(verifieri18n.NoopTranslator{})
	got := tr(stdctx.Background(), "internal_verifier_reserved_placeholder", nil)
	if got != "internal_verifier_reserved_placeholder" {
...
func TestNoopTranslator_TPluralEchoesID(t *testing.T) {
	got, err := verifieri18n.NoopTranslator{}.TPlural(stdctx.Background(), "internal_verifier_reserved_placeholder", 7, nil)
	if err != nil {
