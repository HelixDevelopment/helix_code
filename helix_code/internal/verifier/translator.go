...
var (
	...
	// loads, or verifieri18n.NoopTranslator{} (loud message-ID echo) if it
	// does not. SetTranslator(nil) restores THIS — "nil = restore default"
	// means "restore correct prose", not "revert to raw-key echo".
	defaultTranslator verifieri18n.Translator = verifieri18n.NoopTranslator{}
...
func init() {
	tr, err := verifieri18n.NewTranslator()
	if err != nil {
		fmt.Fprintf(os.Stderr,
			"internal/verifier i18n: embedded-bundle translator load failed; "+
