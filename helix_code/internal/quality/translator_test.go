// Unit tests for the internal/quality package-level translator +
// tr() helper (CONST-046 round-223 §11.4 anti-bluff sweep,
// 2026-05-19).
//
...
// (unit tests only).
//
// Round-223 status: NO-OP INFRA round. The audit gate reports zero
// CONST-046 hardcoded-content violations in internal/quality/ at HEAD,
// so there are no real call sites to exercise yet. These tests
// exercise the seam itself (Translator interface contract,
...
func TestTr_DefaultsToNoopTranslator(t *testing.T) {
	resetTranslator(t)
	got := tr(stdctx.Background(), "internal_quality_reserved_placeholder", nil)
	if got == "internal_quality_reserved_placeholder" || got == "" {
...
func TestTr_UsesInjectedTranslator(t *testing.T) {
	...
	got := tr(stdctx.Background(), "internal_quality_reserved_placeholder", nil)
	want := "<TR:internal_quality_reserved_placeholder>"
	if got != want {
...
func TestTr_TranslatorErrorReturnsMessageID(t *testing.T) {
	...
	got := tr(stdctx.Background(), "internal_quality_reserved_placeholder", nil)
	if got != "internal_quality_reserved_placeholder" {
...
func TestSetTranslator_AcceptsNoopExplicit(t *testing.T) {
	...
	SetTranslator(qualityi18n.NoopTranslator{})
	got := tr(stdctx.Background(), "internal_quality_reserved_placeholder", nil)
	if got != "internal_quality_reserved_placeholder" {
...
func TestNoopTranslator_TPluralEchoesID(t *testing.T) {
	got, err := qualityi18n.NoopTranslator{}.TPlural(stdctx.Background(), "internal_quality_reserved_placeholder", 7, nil)
	if err != nil {
