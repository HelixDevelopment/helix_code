// translator.go — CONST-046 message-ID resolver seam for internal/continua.
//
// Round-230 §11.4 anti-bluff sweep (2026-05-19). Mirrors the
...
import (
	...
	"dev.helix.code/internal/continua/i18n"
)
...
// migration minimally invasive — the continua call sites are cheap
// reads in lookup-style helpers (Description, Complete suggestion
// construction), so the package-level seam keeps the cost behind a
// single pointer fetch rather than threading through every method
// signature.
//
...
func init() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr,
			"internal/continua i18n: embedded-bundle translator load failed; "+
