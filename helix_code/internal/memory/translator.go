...
// internal/memory.
//
// Round-165 §11.4 anti-bluff sweep (2026-05-19). Mirrors the
// "consumer defines its own Translator + tr() helper" pattern used
// by every other CONST-046-migrated package in this codebase
// (rounds 93/94/95/96/108/131/134/136-164, most recently
// internal/hardware 158, internal/event 157, internal/focus 156,
...
// migration minimally invasive — MemoryManager methods are called
// from many sites (and even from package-level helpers like
// FromGlobalManager) and threading a struct-scoped translator would
...
var (
	...
	// loads, or memoryi18n.NoopTranslator{} (loud message-ID echo) if it
	// does not. SetTranslator(nil) restores THIS — "nil = restore default"
	// means "restore correct prose", not "revert to raw-key echo".
	defaultTranslator memoryi18n.Translator = memoryi18n.NoopTranslator{}
	translator        memoryi18n.Translator = memoryi18n.NoopTranslator{}
)
...
func init() {
	tr, err := memoryi18n.NewTranslator()
	if err != nil {
		fmt.Fprintf(os.Stderr,
			"internal/memory i18n: embedded-bundle translator load failed; "+
...
func SetTranslator(tr memoryi18n.Translator) {
	translatorMu.Lock()
...
func tr(ctx stdctx.Context, msgID string, data map[string]any) (result string) {
	...
	if active == nil {
		active = memoryi18n.NoopTranslator{}
	}
