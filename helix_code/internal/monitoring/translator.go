...
import (
	...
	"dev.helix.code/internal/monitoring/i18n"
)
...
// migration minimally invasive — Monitor methods already take their
// own contexts (or use context.Background() at the periodic-loop
// boundary), so threading a struct-scoped translator would inflate
// the diff without behavioural benefit.
//
// HXC-014b §11.4.85: both reads (tr) and writes (SetTranslator) of
// this package-level seam MUST be guarded by translatorMu — Monitor
// emits strings from periodic background loops while SetTranslator
// may be re-invoked at boot/reconfiguration, so an unguarded access
// is a data race (caught by `go test -race`).
var (
...
func init() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr,
			"internal/monitoring i18n: embedded-bundle translator load failed; "+
