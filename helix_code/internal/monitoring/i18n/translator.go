// Package i18n declares internal/monitoring's hardcoded-content
// abstraction per CONST-046 (round-166 §11.4 anti-bluff sweep,
// 2026-05-19).
//
// internal/monitoring hosts the Monitor + Collector + periodic
// collection surface. Every user-visible monitoring message (log
// line, health-check error, collector-failure notice) reaches end
// users via CLI output, server logs, dashboards, and operator
// alerts — therefore every literal string is a CONST-046 candidate.
//
// We follow the "consumer defines its own Translator interface"
// pattern established by prior rounds (93/94/95/96/108/131/134/
// 136-145/146/147/...) for three reasons: (1) uniform pattern
...
// dragging in pkg/i18n's bundle-loading, (3) internal/monitoring
// could plausibly be extracted into its own submodule for reuse by
// other Helix services without restructuring the i18n surface.
//
// The wire-in path at boot is: the consuming binary constructs an
// *i18n.Localizer (loaded with the active.en.yaml bundle from
// internal/monitoring/i18n/bundles), wraps it in
// *i18nadapter.Translator, and stores it via
// monitoring.SetTranslator. The package-level tr() helper resolves
...
// Translator is the contract internal/monitoring uses for every
...
// have not yet wired a real Translator. Production paths MUST
// inject a real Translator (helix_code wires *i18nadapter.Translator
// at boot).
