// Unit tests for the internal/monitoring Translator interface +
// NoopTranslator default. Mocks ALLOWED per CONST-050(A) (unit
// tests only).
package i18n
...
func TestNoopTranslator_T_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.T(context.Background(), "internal_monitoring_no_collectors_registered", nil)
	if err != nil {
		t.Fatalf("NoopTranslator.T returned error: %v", err)
	}
	if got != "internal_monitoring_no_collectors_registered" {
...
func TestNoopTranslator_TPlural_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.TPlural(context.Background(), "internal_monitoring_failed_collect_metrics_named", 2, nil)
	if err != nil {
		t.Fatalf("NoopTranslator.TPlural returned error: %v", err)
	}
	if got != "internal_monitoring_failed_collect_metrics_named" {
...
func TestNoopTranslator_T_IgnoresTemplateData(t *testing.T) {
	...
	// interpolated). If NoopTranslator ever silently interpolated and
	// returned a non-ID string, the assert below would catch it.
	tr := NoopTranslator{}
	got, err := tr.T(context.Background(), "internal_monitoring_failed_collect_metrics_named", map[string]any{"Name": "cpu", "Err": "boom"})
	if err != nil {
		t.Fatalf("NoopTranslator.T returned error: %v", err)
	}
	if got != "internal_monitoring_failed_collect_metrics_named" {
...
func TestFakeTranslator_T_WrapsID(t *testing.T) {
	...
	want := "<TRANSLATED:internal_monitoring_no_collectors_registered>"
	if got != want {
...
func TestFakeTranslator_T_FailOnID(t *testing.T) {
	tr := fakeTranslator{failOnID: "internal_monitoring_no_collectors_registered"}
	if _, err := tr.T(context.Background(), "internal_monitoring_no_collectors_registered", nil); err == nil {
		t.Fatal("fakeTranslator.T expected to fail for matching ID, got nil error")
	}
	// Non-matching IDs should still succeed.
	if _, err := tr.T(context.Background(), "internal_monitoring_failed_collect_metrics_named", nil); err != nil {
