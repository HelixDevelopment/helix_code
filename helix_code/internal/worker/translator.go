// translator.go — CONST-046 message-ID resolver seam for internal/worker.
//
// Round-184 §11.4 anti-bluff sweep (2026-05-19). Mirrors the
// "consumer defines its own Translator + tr() helper" pattern used
// by every other CONST-046-migrated package in this codebase
// (rounds 93/94/95/96/108/131/134/136-183).
package worker
...
import (
	...
	"dev.helix.code/internal/worker/i18n"
)
...
// migration minimally invasive — WorkerManager / DistributedWorkerManager
// / WorkerPool already take a context.Context on every method, so
...
// with tr() (a read) from worker-pool goroutines, so both accesses MUST
