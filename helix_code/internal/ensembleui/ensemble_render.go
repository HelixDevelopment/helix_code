// Package ensembleui holds PURE display-formatting helpers for rendering an
// LLM ensemble's per-member answers and an agentic tool-call trace into plain
// []string display lines.
//
// These functions perform NO I/O, NO network, and depend on NO UI-toolkit
// types (no tview, no Fyne). They take plain data — the ensemble
// ProviderMetadata map produced by internal/llm/ensemble_provider.go and a
// lightweight tool-trace slice — and return []string display lines a caller
// renders however it likes (a tview TextView, a Fyne widget, an HTTP body,
// stdout). Keeping them pure + decoupled (CONST-051(B)) lets EVERY client
// (terminal UI, desktop GUI, web, …) reuse the SAME multi-member-visibility and
// tool-trace rendering without import cycles or duplicated logic (§11.4.74
// reuse-over-reimplement).
//
// Promoted from applications/terminal_ui/ensemble_render.go (which was
// package-main and therefore un-importable) so the desktop client can render
// the identical ensemble panel the TUI does. The TUI now imports this package.
//
