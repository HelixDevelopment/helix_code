...
// migration minimally invasive — CodeEditor / DefaultValidator
// methods are called from many sites and threading a struct-scoped
...
var (
	...
	translator   editori18n.Translator = editori18n.NoopTranslator{}
)
...
func SetTranslator(tr editori18n.Translator) {
	translatorMu.Lock()
	defer translatorMu.Unlock()
	if tr == nil {
		translator = editori18n.NoopTranslator{}
		return
	}
	translator = tr
}
...
func tr(ctx context.Context, msgID string, data map[string]any) (result string) {
	...
	if active == nil {
		active = editori18n.NoopTranslator{}
	}
