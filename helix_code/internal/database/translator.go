// translator.go — CONST-046 message-ID resolver seam for internal/database.
//
// Round-152 §11.4 anti-bluff sweep (2026-05-18). Mirrors the
// "consumer defines its own Translator + tr() helper" pattern used
// by every other CONST-046-migrated package in this codebase
// (rounds 93/94/95/96/108/131/134/136-151).
package database
...
import (
	...
	"dev.helix.code/internal/database/i18n"
)
...
// NOT i18n.NoopTranslator{}. Root cause of HXC-097: Database errors
// such as "internal_database_ping_failed" surface during connection
// setup, which runs on entry paths that never reach the central
// i18nwiring.WireAll() (it is gated behind cmd/cli's interactive-REPL
// subsystem cluster, and the server / config tools never call it), so
// the package ran on NoopTranslator{} and users saw the raw message-ID
// key instead of resolved + interpolated prose. Defaulting to the real
// bundle makes the package emit correct prose out-of-the-box on EVERY
// entry path. A consumer-injected Translator (i18nwiring.WireAll /
// direct SetTranslator) still takes precedence — it simply re-injects an
// equivalent (or locale-specific) translator over this default.
//
...
// migration minimally invasive — Database methods are called from
// many sites and the connection pool is process-wide; threading a
...
func init() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr,
			"internal/database i18n: embedded-bundle translator load failed; "+
