// Package i18n declares internal/database's hardcoded-content
// abstraction per CONST-046 (round-152 §11.4 anti-bluff sweep,
// 2026-05-18). Mirrors the "consumer defines its own Translator
// interface" pattern of rounds 93..151 (most recently internal/context
// 151, internal/config 150, internal/commands 149, internal/cognee
// 148, internal/agent 147, internal/auth 146). Wire-in path at boot:
// consuming binary builds an *i18n.Localizer (loaded with
// active.en.yaml from internal/database/i18n/bundles), wraps it in
// *i18nadapter.Translator, stores via database.SetTranslator. The
// package-level tr() helper falls back to NoopTranslator{} when not
// wired — loud message-ID echo, never silent swallow (which would be
// a §11.4 PASS-bluff at the i18n layer).
package i18n

import "context"

// Translator is the contract internal/database uses for every
