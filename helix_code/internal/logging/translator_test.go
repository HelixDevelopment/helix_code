// Unit tests for the internal/logging package-level translator +
// tr() helper (CONST-046 round-162 §11.4 anti-bluff sweep,
// 2026-05-19).
//
...
// (unit tests only).
//
// Round-162 status: NO-OP INFRA round. The audit gate reports zero
// CONST-046 hardcoded-content violations in internal/logging/ at HEAD,
...
func TestTr_DefaultsToNoopTranslator(t *testing.T) {
	resetTranslator(t)
	got := tr(stdctx.Background(), "internal_logging_reserved_placeholder", nil)
	if got == "internal_logging_reserved_placeholder" || got == "" {
...
func TestTr_UsesInjectedTranslator(t *testing.T) {
	...
	got := tr(stdctx.Background(), "internal_logging_reserved_placeholder", nil)
	want := "<TR:internal_logging_reserved_placeholder>"
	if got != want {
...
func TestNoopTranslator_TPluralEchoesID(t *testing.T) {
	got, err := loggingi18n.NoopTranslator{}.TPlural(stdctx.Background(), "internal_logging_reserved_placeholder", 7, nil)
	if err != nil {
