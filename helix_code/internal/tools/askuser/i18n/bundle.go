// bundle.go — boot-time Translator constructor for internal/tools/askuser
// (HXC-036 Phase 1, 2026-05-29).
//
// The CONST-046 round-440 migration created the Translator interface +
...
// saw raw message-ID keys (e.g. "askuser_prompt_invalid_choice_hint")
// instead of resolved + interpolated text. NewTranslator closes that gap:
// it embeds the active.en.yaml bundle, loads it into a pkg/i18n *Bundle,
// constructs an *i18n.Localizer, and wraps it in an *i18nadapter.Translator
// that structurally satisfies this package's local Translator interface.
//
...
// injects the result via askuser.SetTranslator.
//
...
func NewTranslator(langs ...string) (Translator, error) {
	...
	if err := bundle.LoadMessageFileFS(activeBundleFS, activeBundlePath); err != nil {
		return nil, fmt.Errorf("askuser i18n: load embedded %q: %w", activeBundlePath, err)
	}
	if len(langs) == 0 {
