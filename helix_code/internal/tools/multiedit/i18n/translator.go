// Package i18n declares internal/tools/multiedit's
// hardcoded-content abstraction per CONST-046 (round-428 §11.4
// anti-bluff sweep, 2026-05-20). Mirrors the "consumer defines its
// own Translator interface" pattern of internal/tools/confirmation
// (round-382) and every prior CONST-046-migrated package.
//
// Wire-in path at boot: consuming binary builds an *i18n.Localizer
// (loaded with active.en.yaml from
// internal/tools/multiedit/i18n/bundles), wraps it in
// *i18nadapter.Translator, stores via multiedit.SetTranslator. The
// package-level trc() helper falls back to NoopTranslator{} when
...
// Translator is the contract internal/tools/multiedit uses for
// every CONST-046-migrated user-facing string.
