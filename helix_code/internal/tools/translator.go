// translator.go — CONST-046 message-ID resolver seam for internal/tools.
//
// Round-181 §11.4 anti-bluff sweep (2026-05-19). Mirrors the
// "consumer defines its own Translator + tr() helper" pattern used
// by every other CONST-046-migrated package in this codebase
// (rounds 93/94/95/96/108/131/134/136-178).
package tools
...
import (
	...
	"dev.helix.code/internal/tools/i18n"
)
...
// migration minimally invasive — Tool.Description() methods are
// receivers with no context parameter and are called from many
// sites; threading a struct-scoped translator would inflate the
...
func init() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr,
			"internal/tools i18n: embedded-bundle translator load failed; "+
