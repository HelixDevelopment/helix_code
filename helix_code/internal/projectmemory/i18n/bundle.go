// bundle.go — boot-time Translator constructor for internal/projectmemory
...
func NewTranslator(langs ...string) (Translator, error) {
	...
	if err := bundle.LoadMessageFileFS(activeBundleFS, activeBundlePath); err != nil {
		return nil, fmt.Errorf("internal/projectmemory i18n: load embedded %q: %w", activeBundlePath, err)
	}
	if len(langs) == 0 {
