// Unit tests for the internal/rules Translator interface +
...
func TestNoopTranslator_T_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.T(context.Background(), "internal_rules_rule_name_cannot_be_empty", nil)
	if err != nil {
		t.Fatalf("NoopTranslator.T returned error: %v", err)
	}
	if got != "internal_rules_rule_name_cannot_be_empty" {
...
func TestNoopTranslator_TPlural_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.TPlural(context.Background(), "internal_rules_rule_content_cannot_be_empty", 3, nil)
	if err != nil {
		t.Fatalf("NoopTranslator.TPlural returned error: %v", err)
	}
	if got != "internal_rules_rule_content_cannot_be_empty" {
...
func TestNoopTranslator_T_IgnoresTemplateData(t *testing.T) {
	// Anti-bluff (paired mutation per §11.4): NoopTranslator returns
	// the raw ID even when templateData is provided. This guarantees
	// a test using NoopTranslator can detect a non-i18n call site by
	// the literal remaining unchanged (sentinel = raw ID, not
	// interpolated). If the NoopTranslator implementation ever
	// silently interpolated and returned a non-ID string, the assert
	// below would catch it.
	tr := NoopTranslator{}
	got, err := tr.T(context.Background(), "internal_rules_failed_to_load_workspace_rules", map[string]any{"Err": "boom"})
	if err != nil {
		t.Fatalf("NoopTranslator.T returned error: %v", err)
	}
	if got != "internal_rules_failed_to_load_workspace_rules" {
...
func TestFakeTranslator_T_WrapsID(t *testing.T) {
	tr := fakeTranslator{}
	got, err := tr.T(context.Background(), "internal_rules_rule_pattern_cannot_be_empty", nil)
	if err != nil {
		t.Fatalf("fakeTranslator.T returned error: %v", err)
	}
	want := "<TRANSLATED:internal_rules_rule_pattern_cannot_be_empty>"
	if got != want {
...
func TestFakeTranslator_T_FailOnID(t *testing.T) {
	tr := fakeTranslator{failOnID: "internal_rules_rule_name_cannot_be_empty"}
	if _, err := tr.T(context.Background(), "internal_rules_rule_name_cannot_be_empty", nil); err == nil {
		t.Fatal("fakeTranslator.T expected to fail for matching ID, got nil error")
	}
	// Non-matching IDs should still succeed.
	if _, err := tr.T(context.Background(), "internal_rules_rule_content_cannot_be_empty", nil); err != nil {
