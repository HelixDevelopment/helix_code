// translator.go — CONST-046 message-ID resolver seam for internal/session.
//
// Round-178 §11.4 anti-bluff sweep (2026-05-19). Mirrors the
// "consumer defines its own Translator + tr() helper" pattern used
// by every other CONST-046-migrated package in this codebase
// (rounds 93/94/95/96/108/131/134/136-177).
package session

import (
	stdctx "context"

	"dev.helix.code/internal/session/i18n"
)
...
// migration minimally invasive — Session/Manager methods are called
// from many sites and the manager is process-wide; threading a
