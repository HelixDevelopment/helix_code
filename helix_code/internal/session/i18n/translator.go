// Package i18n declares internal/session's hardcoded-content
// abstraction per CONST-046 (round-178 §11.4 anti-bluff sweep,
// 2026-05-19). Mirrors the "consumer defines its own Translator
// interface" pattern of rounds 93..177 (most recently
// internal/database 152). Wire-in path at boot: consuming binary
// builds an *i18n.Localizer (loaded with active.en.yaml from
// internal/session/i18n/bundles), wraps it in
// *i18nadapter.Translator, stores via session.SetTranslator. The
...
// Translator is the contract internal/session uses for every
