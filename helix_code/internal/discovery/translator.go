...
// internal/discovery.
//
// Round-154 §11.4 anti-bluff sweep (2026-05-18). Mirrors the
// "consumer defines its own Translator + tr() helper" pattern used
// by every other CONST-046-migrated package in this codebase
// (rounds 93/94/95/96/108/131/134/136-153, most recently
// internal/deployment 153 and internal/database 152).
package discovery
...
// migration minimally invasive — DiscoveryClient methods are called
...
var (
	...
	translator   discoveryi18n.Translator = discoveryi18n.NoopTranslator{}
)
...
func SetTranslator(tr discoveryi18n.Translator) {
	translatorMu.Lock()
	defer translatorMu.Unlock()
	if tr == nil {
		translator = discoveryi18n.NoopTranslator{}
		return
	}
	translator = tr
}
...
func tr(ctx stdctx.Context, msgID string, data map[string]any) (result string) {
	...
	if active == nil {
		active = discoveryi18n.NoopTranslator{}
	}
