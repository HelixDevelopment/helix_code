// Unit tests for the internal/providers Translator interface +
...
func TestNoopTranslator_T_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.T(context.Background(), "internal_providers_ai_provider_not_found", nil)
	if err != nil {
		t.Fatalf("NoopTranslator.T returned error: %v", err)
	}
	if got != "internal_providers_ai_provider_not_found" {
...
func TestNoopTranslator_TPlural_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.TPlural(context.Background(), "internal_providers_conversation_not_found", 3, nil)
	if err != nil {
...
func TestNoopTranslator_T_IgnoresTemplateData(t *testing.T) {
	...
	// catch it.
	tr := NoopTranslator{}
	got, err := tr.T(context.Background(), "internal_providers_personality_not_found", map[string]any{"ID": "p1"})
	if err != nil {
		t.Fatalf("NoopTranslator.T returned error: %v", err)
	}
...
func TestFakeTranslator_T_WrapsID(t *testing.T) {
	tr := fakeTranslator{}
	got, err := tr.T(context.Background(), "internal_providers_vector_not_found", nil)
	if err != nil {
		t.Fatalf("fakeTranslator.T returned error: %v", err)
	}
	want := "<TRANSLATED:internal_providers_vector_not_found>"
	if got != want {
...
func TestFakeTranslator_T_FailOnID(t *testing.T) {
	tr := fakeTranslator{failOnID: "internal_providers_provider_not_initialized"}
	if _, err := tr.T(context.Background(), "internal_providers_provider_not_initialized", nil); err == nil {
		t.Fatal("fakeTranslator.T expected to fail for matching ID, got nil error")
	}
	// Non-matching IDs should still succeed.
	if _, err := tr.T(context.Background(), "internal_providers_vector_not_found", nil); err != nil {
