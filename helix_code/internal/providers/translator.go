...
// migration minimally invasive — AIIntegration + ConversationManager
// + PersonalityManager + VectorIntegration methods already take
// their own contexts, so threading a struct-scoped translator would
// inflate the diff without behavioural benefit.
//
// HXC-014b §11.4.85 fix: SetTranslator (a write) may run concurrently
// with tr() (a read) from background goroutines, so both accesses MUST
// be guarded by translatorMu — otherwise the concurrent read/write is a
// data race (caught by `go test -race`), a §11.4.85(B) state-corruption
// defect.
var (
...
// injected implementation) MUST NOT crash the emitting goroutine — the
// recover() below isolates such a panic and degrades to the message ID.
