// Unit tests for the internal/cognee/i18n Translator + NoopTranslator
// (CONST-046 round-148 §11.4 anti-bluff sweep, 2026-05-18).
//
...
func TestNoopTranslator_TReturnsIDUnchanged(t *testing.T) {
	n := NoopTranslator{}
	got, err := n.T(context.Background(), "internal_cognee_some_id", nil)
	if err != nil {
		t.Fatalf("NoopTranslator.T returned error %v, want nil", err)
	}
	if got != "internal_cognee_some_id" {
...
func TestNoopTranslator_TIgnoresTemplateData(t *testing.T) {
	...
	n := NoopTranslator{}
	got, _ := n.T(context.Background(), "internal_cognee_failed_x", map[string]any{"Err": "boom"})
	if got != "internal_cognee_failed_x" {
...
func TestNoopTranslator_TPluralReturnsIDUnchanged(t *testing.T) {
	n := NoopTranslator{}
	got, err := n.TPlural(context.Background(), "internal_cognee_plural_id", 5, nil)
	if err != nil {
		t.Fatalf("NoopTranslator.TPlural returned error %v, want nil", err)
	}
	if got != "internal_cognee_plural_id" {
