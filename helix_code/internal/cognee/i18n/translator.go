// Package i18n declares internal/cognee's hardcoded-content abstraction
// per CONST-046 (round-148 §11.4 anti-bluff sweep, 2026-05-18).
//
// internal/cognee hosts the Cognee knowledge-graph integration surface
// — CogneeManager methods and HTTP handlers under /cognee/* return
// error messages that reach end users via CLI output, API responses,
// and web/desktop/mobile clients. Every literal error string is
// therefore a CONST-046 candidate.
//
// We follow the "consumer defines its own Translator interface"
// pattern established by rounds 93/94/95/96/108/131/134/136-147
// (Lazy / SelfImprove / HelixLLM / harmony_os / helix_config /
// helixllm / cli / server / desktop / terminal_ui / ios / android /
// aurora_os / config_test / security_test / security_fix /
// performance_optimization / security_fix_standalone / internal/auth /
// internal/agent) for three reasons: (1) uniform pattern across the
...
// pkg/i18n's bundle-loading, (3) internal/cognee could plausibly be
// extracted into its own submodule for reuse by other Helix services
...
// internal/cognee/i18n/bundles), wraps it in *i18nadapter.Translator,
// and stores it via cognee.SetTranslator. The package-level tr()
// helper resolves message IDs through this interface and falls back
// to NoopTranslator{} when no real translator has been wired — loud
...
// Translator is the contract internal/cognee uses for every
