...
// internal/logo.
//
// Round-163 §11.4 anti-bluff sweep (2026-05-19). Mirrors the
...
// migration minimally invasive — LogoProcessor methods are called
// from many sites (CLI, asset-pipeline, theme-gen) and threading a
...
var (
	...
	translator   logoi18n.Translator = logoi18n.NoopTranslator{}
)
...
func SetTranslator(tr logoi18n.Translator) {
	translatorMu.Lock()
	defer translatorMu.Unlock()
	if tr == nil {
		translator = logoi18n.NoopTranslator{}
		return
	}
	translator = tr
}
...
func tr(ctx stdctx.Context, msgID string, data map[string]any) (result string) {
	...
	if active == nil {
		active = logoi18n.NoopTranslator{}
	}
