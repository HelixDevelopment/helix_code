...
func (c *ReportBugCommand) Execute(ctx context.Context, cmdCtx *commands.CommandContext) (*commands.CommandResult, error) {
	...
	if title == "" {
		title = tr(ctx, "builtin_reportbug_title_prefix", map[string]any{"Description": description})
	}
	...
	// Format bug report
	bugReport := formatBugReport(ctx, title, description, systemInfo, recentLogs, cmdCtx.ChatHistory)
...
func formatBugReport(ctx context.Context, title, description string, systemInfo map[string]string, logs string, history []commands.ChatMessage) string {
	var sb strings.Builder

	sb.WriteString(tr(ctx, "builtin_reportbug_section_description", nil))
	sb.WriteString("\n\n")
	sb.WriteString(description)
	sb.WriteString("\n\n")

	sb.WriteString(tr(ctx, "builtin_reportbug_section_system_info", nil))
	sb.WriteString("\n\n")
	sb.WriteString("```\n")
	...
	sb.WriteString(tr(ctx, "builtin_reportbug_section_reproduction", nil))
	sb.WriteString("\n\n")
	sb.WriteString(extractReproductionSteps(ctx, history))
	sb.WriteString("\n\n")

	if logs != "" {
		sb.WriteString(tr(ctx, "builtin_reportbug_section_logs", nil))
		sb.WriteString("\n\n")
		sb.WriteString("```\n")
		sb.WriteString(logs)
		sb.WriteString("\n```\n\n")
	...
	sb.WriteString(tr(ctx, "builtin_reportbug_section_expected", nil))
	sb.WriteString("\n\n")
	sb.WriteString(tr(ctx, "builtin_reportbug_placeholder_expected", nil))
	sb.WriteString("\n\n")

	sb.WriteString(tr(ctx, "builtin_reportbug_section_actual", nil))
	sb.WriteString("\n\n")
	sb.WriteString(tr(ctx, "builtin_reportbug_placeholder_actual", nil))
	sb.WriteString("\n\n")

	sb.WriteString("---\n")
	sb.WriteString(tr(ctx, "builtin_reportbug_footer", nil))
	sb.WriteString("\n")
...
func extractReproductionSteps(ctx context.Context, history []commands.ChatMessage) string {
	if len(history) == 0 {
		return tr(ctx, "builtin_reportbug_placeholder_repro_steps", nil) + "\n"
	}
