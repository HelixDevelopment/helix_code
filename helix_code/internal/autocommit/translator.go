// translator.go — CONST-046 message-ID resolver seam for internal/autocommit.
//
// Round-229 §11.4 anti-bluff sweep (2026-05-19). Mirrors the
// "consumer defines its own Translator + tr() helper" pattern used
// by every other CONST-046-migrated package in this codebase
// (rounds 221..228; reference round-221 = internal/approval).
package autocommit
...
import (
	...
	"dev.helix.code/internal/autocommit/i18n"
)
...
// migration minimally invasive — AutoCommitter.MaybeCommit is on
// the post-tool-Execute hot path; threading a translator through
// every CommitContext would inflate cost. The package-level seam
...
func init() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr,
			"internal/autocommit i18n: embedded-bundle translator load failed; "+
