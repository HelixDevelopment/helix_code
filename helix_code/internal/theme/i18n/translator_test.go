// Unit tests for the internal/theme Translator interface + NoopTranslator
...
func TestNoopTranslator_T_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.T(context.Background(), "internal_theme_placeholder", nil)
	if err != nil {
		t.Fatalf("NoopTranslator.T returned error: %v", err)
	}
	if got != "internal_theme_placeholder" {
...
func TestNoopTranslator_TPlural_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.TPlural(context.Background(), "internal_theme_placeholder", 3, nil)
	if err != nil {
...
func TestNoopTranslator_T_IgnoresTemplateData(t *testing.T) {
	...
	tr := NoopTranslator{}
	got, err := tr.T(context.Background(), "internal_theme_placeholder", map[string]any{"Path": "/etc/x"})
	if err != nil {
		t.Fatalf("NoopTranslator.T returned error: %v", err)
	}
	if got != "internal_theme_placeholder" {
...
func TestNoopTranslator_T_AllBundleIDs(t *testing.T) {
	...
	//
	// Round-238 scaffold: bundle currently declares one placeholder
	// ID; this test extends as real IDs land in subsequent migration
	// rounds for internal/theme.
	tr := NoopTranslator{}
	ids := []string{
		"internal_theme_placeholder",
	}
	for _, id := range ids {
...
func TestFakeTranslator_T_WrapsID(t *testing.T) {
	...
	want := "<TRANSLATED:internal_theme_placeholder>"
	if got != want {
...
func TestFakeTranslator_T_FailOnID(t *testing.T) {
	tr := fakeTranslator{failOnID: "internal_theme_placeholder"}
	if _, err := tr.T(context.Background(), "internal_theme_placeholder", nil); err == nil {
		t.Fatal("fakeTranslator.T expected to fail for matching ID, got nil error")
	}
	// Non-matching IDs should still succeed.
	if _, err := tr.T(context.Background(), "internal_theme_other", nil); err != nil {
