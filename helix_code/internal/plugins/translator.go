...
// internal/plugins.
//
// Round-234 §11.4 anti-bluff sweep (2026-05-19). Mirrors the
// "consumer defines its own Translator + tr() helper" pattern used
// by every other CONST-046-migrated package in this codebase
// (rounds 93/94/95/96/108/131/134/136-146/221/222/223/224/225/226/
// 228/233/etc.).
package plugins
...
import (
	...
	"dev.helix.code/internal/plugins/i18n"
)
...
// migration minimally invasive — ExecutePlugin is a package-level
// function (not a method on a struct that already threads context),
// and the manifest/loader/registry surfaces are all package-level
// types that callers already construct without an injected
// translator parameter. The package-level seam matches the
// established pattern from auth/mcp/secrets/etc.
//
...
func init() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr,
			"internal/plugins i18n: embedded-bundle translator load failed; "+
