// Package i18n declares the HelixCode HTTP server's hardcoded-content
// abstraction per CONST-046 (round-134 §11.4 anti-bluff sweep,
// 2026-05-18).
//
// cmd/server is the production HTTP server entry point living inside
// the dev.helix.code module (NOT an externally-consumable submodule),
// so it COULD import dev.helix.code/pkg/i18nadapter directly. We
// nonetheless follow the "consumer defines its own Translator
// interface" pattern established by rounds 93/94/95/96/108/131
// (Lazy / SelfImprove / HelixLLM / harmony_os / helix_config / cli)
...
// loading, (3) future extraction of cmd/server into its own
...
// SetTranslator. main() reaches the translator via the package-level
// tr() helper because the startup/shutdown flow uses direct
// fmt.Printf / log.Printf calls inline — global injection matches
// log's own use of package-level state and keeps the migration
// minimally invasive.
package i18n

import "context"

// Translator is the contract cmd/server uses for every CONST-046-
