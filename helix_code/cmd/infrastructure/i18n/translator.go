// Package i18n declares the infrastructure CLI's hardcoded-content
// abstraction per CONST-046 (round-455 §11.4 anti-bluff sweep,
// 2026-05-20).
//
// infrastructure is a tiny CLI living inside the dev.helix.code
// module (NOT an externally-consumable submodule), so it COULD import
...
// by every prior CONST-046-migrated cmd/ subdirectory (helix_config,
// config_test, security_test, security_fix, cli, server,
// performance_optimization, ...) for three reasons: (1) a uniform
...
// extraction of infrastructure into its own submodule would not
// require restructuring the i18n surface.
//
// The wire-in path at boot is: infrastructure constructs an
...
// Translator is the contract the infrastructure CLI uses for every
