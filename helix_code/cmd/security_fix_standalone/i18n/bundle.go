// bundle.go — boot-time Translator constructor for cmd/security_fix_standalone
...
func NewTranslator(langs ...string) (Translator, error) {
	...
	if err := bundle.LoadMessageFileFS(activeBundleFS, activeBundlePath); err != nil {
		return nil, fmt.Errorf("cmd/security_fix_standalone i18n: load embedded %q: %w", activeBundlePath, err)
	}
	if len(langs) == 0 {
