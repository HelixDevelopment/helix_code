// Package i18n declares security_fix_standalone's hardcoded-content
// abstraction per CONST-046 (round-145 §11.4 anti-bluff sweep,
// 2026-05-18).
//
// security_fix_standalone is a standalone-variant CLI living inside
// the dev.helix.code module (NOT an externally-consumable submodule)
// and is a sibling of cmd/security_fix (whose i18n surface landed in
// round 143 at bbbf121). It COULD import dev.helix.code/pkg/i18nadapter
// directly, but we deliberately follow the "consumer defines its own
// Translator interface" pattern established by rounds 93/94/95/96/108/
// 131/134/136-143 (Lazy / SelfImprove / HelixLLM / harmony_os /
...
// security_fix_standalone into its own submodule (or merge with
// security_fix) would not require restructuring the i18n surface.
//
// The wire-in path at boot is: security_fix_standalone constructs an
...
// Translator is the contract security_fix_standalone uses for every
