// Unit tests for the config_test Translator interface + NoopTranslator
...
func TestNoopTranslator_T_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.T(context.Background(), "config_test_header_title", nil)
	if err != nil {
...
func TestNoopTranslator_TPlural_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.TPlural(context.Background(), "config_test_initial_loaded", 3, nil)
	if err != nil {
		t.Fatalf("NoopTranslator.TPlural returned error: %v", err)
	}
	if got != "config_test_initial_loaded" {
...
func TestNoopTranslator_T_IgnoresTemplateData(t *testing.T) {
	...
	tr := NoopTranslator{}
	got, err := tr.T(context.Background(), "config_test_config_path", map[string]any{"Path": "/etc/helix/config.yaml"})
	if err != nil {
		t.Fatalf("NoopTranslator.T returned error: %v", err)
	}
	if got != "config_test_config_path" {
...
func TestFakeTranslator_T_WrapsID(t *testing.T) {
	...
	want := "<TRANSLATED:config_test_header_title>"
	if got != want {
...
func TestFakeTranslator_T_FailOnID(t *testing.T) {
	tr := fakeTranslator{failOnID: "config_test_initial_loaded"}
	if _, err := tr.T(context.Background(), "config_test_initial_loaded", nil); err == nil {
		t.Fatal("fakeTranslator.T expected to fail for matching ID, got nil error")
	}
	// Non-matching IDs should still succeed.
	if _, err := tr.T(context.Background(), "config_test_header_title", nil); err != nil {
