// Unit tests for the security_fix Translator interface + NoopTranslator
...
func TestNoopTranslator_T_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.T(context.Background(), "security_fix_banner_start", nil)
	if err != nil {
...
func TestNoopTranslator_TPlural_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.TPlural(context.Background(), "security_fix_result_success", 3, nil)
	if err != nil {
...
func TestNoopTranslator_T_IgnoresTemplateData(t *testing.T) {
	...
	tr := NoopTranslator{}
	got, err := tr.T(context.Background(), "security_fix_result_failure", map[string]any{"Count": 5})
	if err != nil {
		t.Fatalf("NoopTranslator.T returned error: %v", err)
	}
	if got != "security_fix_result_failure" {
...
func TestFakeTranslator_T_WrapsID(t *testing.T) {
	tr := fakeTranslator{}
	got, err := tr.T(context.Background(), "security_fix_banner_policy", nil)
	if err != nil {
		t.Fatalf("fakeTranslator.T returned error: %v", err)
	}
	want := "<TRANSLATED:security_fix_banner_policy>"
	if got != want {
...
func TestFakeTranslator_T_FailOnID(t *testing.T) {
	tr := fakeTranslator{failOnID: "security_fix_result_success"}
	if _, err := tr.T(context.Background(), "security_fix_result_success", nil); err == nil {
		t.Fatal("fakeTranslator.T expected to fail for matching ID, got nil error")
	}
	// Non-matching IDs should still succeed.
	if _, err := tr.T(context.Background(), "security_fix_banner_start", nil); err != nil {
