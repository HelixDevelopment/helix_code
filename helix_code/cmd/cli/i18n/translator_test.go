// Unit tests for the cli Translator interface + NoopTranslator
...
func TestNoopTranslator_T_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.T(context.Background(), "cli_repl_intro", nil)
	if err != nil {
		t.Fatalf("NoopTranslator.T returned error: %v", err)
	}
	if got != "cli_repl_intro" {
...
func TestNoopTranslator_TPlural_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.TPlural(context.Background(), "cli_workers_total", 3, nil)
	if err != nil {
...
func TestNoopTranslator_T_IgnoresTemplateData(t *testing.T) {
	...
	tr := NoopTranslator{}
	got, err := tr.T(context.Background(), "cli_workers_total", map[string]any{"Count": 42})
	if err != nil {
		t.Fatalf("NoopTranslator.T returned error: %v", err)
	}
	if got != "cli_workers_total" {
...
func TestFakeTranslator_T_WrapsID(t *testing.T) {
	...
	want := "<TRANSLATED:cli_repl_intro>"
	if got != want {
...
func TestFakeTranslator_T_FailOnID(t *testing.T) {
	tr := fakeTranslator{failOnID: "cli_qa_no_sessions"}
	if _, err := tr.T(context.Background(), "cli_qa_no_sessions", nil); err == nil {
		t.Fatal("fakeTranslator.T expected to fail for matching ID, got nil error")
	}
	// Non-matching IDs should still succeed.
	if _, err := tr.T(context.Background(), "cli_repl_intro", nil); err != nil {
