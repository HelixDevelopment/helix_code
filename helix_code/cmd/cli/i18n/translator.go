// Package i18n declares cli's hardcoded-content abstraction
// per CONST-046 (round-131 §11.4 anti-bluff sweep, 2026-05-18).
//
// cli is a CLI tool living inside the dev.helix.code module
...
// by rounds 93/94/95/96/108 for three reasons: (1) uniform pattern
// across the codebase keeps mental overhead low, (2) the Translator
// interface is the natural seam for stubbing in tests without dragging
// in pkg/i18n's bundle-loading, (3) future extraction of cli into its
// own submodule would not require restructuring the i18n surface.
//
// The wire-in path at boot is: cli constructs an *i18n.Localizer
// (loaded with the active.en.yaml bundle from ./bundles), wraps it
// in *i18nadapter.Translator, and stores it via SetTranslator. CLI
// handlers reach the translator via the package-level tr() helper in
// main.go because the legacy CLI handler signatures
// (func(*CLI, context.Context) error) do not support injection of
// additional parameters without restructuring the handler tree.
package i18n

import "context"

// Translator is the contract cli uses for every CONST-046-migrated
// user-facing string. The package-level tr() helper in main.go
// resolves IDs through this interface and falls back to
