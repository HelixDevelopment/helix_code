// bundle.go — boot-time Translator constructor for cmd/performance_optimization
...
func NewTranslator(langs ...string) (Translator, error) {
	...
	if err := bundle.LoadMessageFileFS(activeBundleFS, activeBundlePath); err != nil {
		return nil, fmt.Errorf("cmd/performance_optimization i18n: load embedded %q: %w", activeBundlePath, err)
	}
	if len(langs) == 0 {
