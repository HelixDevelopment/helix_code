...
// TestOpenRouterProviderFullAutomation tests the OpenRouter provider with real API calls
func TestOpenRouterProviderFullAutomation(t *testing.T) {
	// Get API key from environment
	apiKey := os.Getenv("OPENROUTER_API_KEY")
	if apiKey == "" {
		t.Skip("OPENROUTER_API_KEY environment variable not set, skipping real API tests")  // SKIP-OK: #requires-upstream-key
	}
	...
	t.Run("ModelListing", func(t *testing.T) {
		...
		for _, model := range models {
			assert.Equal(t, llm.ProviderTypeOpenRouter, model.Provider)
	...
	// Test basic text generation with free model
	...
	t.Run("ErrorHandling", func(t *testing.T) {
		...
		assert.Contains(t, err.Error(), "OpenRouter API returned status")
	})
...
// TestOpenRouterProviderLoadTest performs load testing with real API
func TestOpenRouterProviderLoadTest(t *testing.T) {
	apiKey := os.Getenv("OPENROUTER_API_KEY")
	if apiKey == "" {
		t.Skip("OPENROUTER_API_KEY environment variable not set, skipping load test")  // SKIP-OK: #requires-upstream-key
	}
...
// TestOpenRouterProviderModelCompatibility tests all available models
func TestOpenRouterProviderModelCompatibility(t *testing.T) {
	apiKey := os.Getenv("OPENROUTER_API_KEY")
	if apiKey == "" {
		t.Skip("OPENROUTER_API_KEY environment variable not set, skipping model compatibility test")  // SKIP-OK: #requires-upstream-key
	}
...
// TestOpenRouterProviderRateLimits tests rate limit handling
func TestOpenRouterProviderRateLimits(t *testing.T) {
	apiKey := os.Getenv("OPENROUTER_API_KEY")
	if apiKey == "" {
		t.Skip("OPENROUTER_API_KEY environment variable not set, skipping rate limit test")  // SKIP-OK: #requires-upstream-key
	}
