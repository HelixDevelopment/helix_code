...
// translators for every importable internal/* package on the harmony_os
// (GUI + nogui) entry path.
//
// The harmony_os binary never called i18nwiring.WireAll(), so the ~60
// internal/* packages it transitively emits user-facing strings through
// (internal/agent, internal/llm, internal/provider, internal/tools/askuser,
// …) ran on their loud NoopTranslator{} raw-key-echo default and real users
// saw raw message-ID keys instead of resolved + interpolated prose — a
// CONST-046 / §11.4 regression on the harmony_os entry path.
//
