...
// Goal A on the harmony_os entry path (2026-06-15).
//
// Proves the harmony_os binary wires the importable internal/* CONST-046
...
func TestHXC099_GoalA_HarmonyOSEntryPathWiresInternalPackages(t *testing.T) {
	...
	if strings.Contains(got, rawKey) {
		t.Fatalf("HXC-099 GoalA REGRESSION: FormatQuestion = %q still leaks raw key %q "+
			"(harmony_os entry path did not call i18nwiring.WireAll at boot)", got, rawKey)
	}
