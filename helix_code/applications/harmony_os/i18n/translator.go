// Package i18n declares harmony_os's hardcoded-content abstraction
// per CONST-046 (round-96 §11.4 anti-bluff sweep, 2026-05-18).
//
// harmony_os is an application living inside the dev.helix.code module
...
// by rounds 93/94/95 (Lazy / SelfImprove / HelixLLM) for three
...
// (3) future extraction of harmony_os into its own submodule would
// not require restructuring the i18n surface.
//
// The wire-in path at boot is: helix_code constructs an
// *i18n.Localizer (loaded with the active.en.yaml bundle from
// ./bundles), wraps it in *i18nadapter.Translator, and passes it to
// the harmony app's constructor as a Translator (structural match —
// no shared package required).
package i18n

import "context"

// Translator is the contract harmony_os uses for every CONST-046-
// migrated user-facing string. Constructors accept a Translator and
// fall back to NoopTranslator{} when nil (loud message-ID echo —
// never silently swallow translation requests, which would be a
