// bundle.go — boot-time Translator constructor for the harmony_os app.
//
...
// the harmony_os app via SetTranslator"). But that boot-time constructor was
// never written for the standalone harmony_os binary (GUI or `nogui`), so
// every run fell back to NoopTranslator{} and the CLI leaked raw message-ID
// keys (e.g. `harmony_os_cli_status_header`, `harmony_os_cli_help_body`)
// instead of resolved text — a §11.4 / CONST-046 PASS-bluff visible on
// `status`/`help`/`version`. This is the harmony_os instance of the systemic
// HXC-095 (desktop) / aurora_os defect. NewTranslator closes the gap,
...
// the harmony_os binary calls this once and injects the result via
// SetTranslator.
//
...
func NewTranslator(langs ...string) (Translator, error) {
	...
	if err := bundle.LoadMessageFileFS(activeBundleFS, activeBundlePath); err != nil {
		return nil, fmt.Errorf("harmony_os i18n: load embedded %q: %w", activeBundlePath, err)
	}
	if len(langs) == 0 {
