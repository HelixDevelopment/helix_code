// bundle.go — boot-time Translator constructor for the desktop app.
//
...
// the desktop app via SetTranslator"). But that boot-time constructor was
// never written for the standalone desktop binary (GUI or `nogui`), so every
// run fell back to NoopTranslator{} and the CLI leaked raw message-ID keys
// (e.g. `desktop_cli_status_header`, `desktop_cli_help_body`) instead of
// resolved text — a §11.4 / CONST-046 PASS-bluff visible on `status`/`help`.
// A second symptom rode along: Printf call sites that pass args to a
// format-string echoed as a bare key (no `%` verbs) emit Go's
// `%!(EXTRA int=0, ...)` noise. NewTranslator closes both gaps, mirroring
...
// the desktop binary calls this once and injects the result via SetTranslator.
//
...
func NewTranslator(langs ...string) (Translator, error) {
	...
	if err := bundle.LoadMessageFileFS(activeBundleFS, activeBundlePath); err != nil {
		return nil, fmt.Errorf("desktop i18n: load embedded %q: %w", activeBundlePath, err)
	}
	if len(langs) == 0 {
