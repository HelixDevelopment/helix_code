...
// Goal A on the terminal_ui entry path (2026-06-15).
//
// Proves the TUI binary wires the importable internal/* CONST-046 packages
// at boot via i18nwiring.WireAll() (from this package's init() in
...
func TestHXC099_GoalA_TerminalUIEntryPathWiresInternalPackages(t *testing.T) {
	...
	if strings.Contains(got, rawKey) {
		t.Fatalf("HXC-099 GoalA REGRESSION: FormatQuestion = %q still leaks raw key %q "+
			"(terminal_ui entry path did not call i18nwiring.WireAll at boot)", got, rawKey)
	}
