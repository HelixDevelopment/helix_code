// Package i18n declares the terminal_ui application's hardcoded-content
// abstraction per CONST-046 (round-137 §11.4 anti-bluff sweep,
// 2026-05-18).
//
// The terminal_ui app is built on tview/tcell and lives inside the
// dev.helix.code module as `package main` (cmd-style entry point), so
// it COULD import dev.helix.code/pkg/i18nadapter directly. We
// nonetheless follow the "consumer defines its own Translator
// interface" pattern established by rounds 93/94/95/96/108/131/134/136
...
// seam for stubbing in tests without dragging in pkg/i18n's
// bundle-loading, (3) future extraction of the terminal_ui app into
// its own submodule would not require restructuring the i18n surface.
//
...
// the terminal_ui app via SetTranslator (structural match — no shared
// package required).
package i18n

import "context"

// Translator is the contract the terminal_ui app uses for every
