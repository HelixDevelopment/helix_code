...
// Goal A on the aurora_os entry path (2026-06-15).
//
// Proves the aurora_os binary wires the importable internal/* CONST-046
...
func TestHXC099_GoalA_AuroraOSEntryPathWiresInternalPackages(t *testing.T) {
	...
	if strings.Contains(got, rawKey) {
		t.Fatalf("HXC-099 GoalA REGRESSION: FormatQuestion = %q still leaks raw key %q "+
			"(aurora_os entry path did not call i18nwiring.WireAll at boot)", got, rawKey)
	}
