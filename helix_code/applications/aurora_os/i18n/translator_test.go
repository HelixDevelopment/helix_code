// Unit tests for the aurora_os Translator interface + NoopTranslator
...
func TestNoopTranslator_T_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.T(context.Background(), "aurora_os_tab_dashboard", nil)
	if err != nil {
		t.Fatalf("NoopTranslator.T returned error: %v", err)
	}
	if got != "aurora_os_tab_dashboard" {
...
func TestNoopTranslator_TPlural_ReturnsID(t *testing.T) {
	tr := NoopTranslator{}
	got, err := tr.TPlural(context.Background(), "aurora_os_tasks_count", 3, nil)
	if err != nil {
		t.Fatalf("NoopTranslator.TPlural returned error: %v", err)
	}
	if got != "aurora_os_tasks_count" {
...
func TestFakeTranslator_T_WrapsID(t *testing.T) {
	...
	want := "<TRANSLATED:aurora_os_tab_dashboard>"
	if got != want {
