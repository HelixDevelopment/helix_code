...
if want_gate G5; then
    ...
    tracked_sensitive=$(
        ...
        grep -vE '^(HelixAgent|cli_agents|cli_agents_resources|Dependencies/LLama_CPP|Dependencies/Ollama|Dependencies/HuggingFace_Hub|HelixQA/tools|panoptic/tests|panoptic/docs)' || true
    )
