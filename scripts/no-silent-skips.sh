...
# helix_agent/MCP/submodules/python-sdk/, helix_qa/tools/opensource/...)
...
VENDORED_PATH_REGEX='helix_agent/MCP/submodules/|helix_qa/tools/opensource/|/python-sdk/|/llama-index/|/llama_index/|/chroma[/_]|/unstructured/|/browser-use/|/atlassian-mcp/|/opensource/'
...
# `SKIP-OK:` that isn't whitespace/end-of-line is a valid marker token,
# so the gate enforces "must have a marker" without dictating the
# specific token alphabet.
violations=$(grep -rnE "$PATTERNS" "${INCLUDES[@]}" "${EXCLUDES[@]}" . 2>/dev/null \
             | grep -v -E 'SKIP-OK: *[^[:space:]]+' \
