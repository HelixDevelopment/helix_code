...
func NewDebuggingAgent(config *config.AgentConfig, provider llm.Provider, toolRegistry *tools.ToolRegistry) (*DebuggingAgent, error) {
	if provider == nil {
	...
	baseAgent := agent.NewBaseAgent("debugging-agent", "Debugging Agent", config)
...
func (a *DebuggingAgent) Initialize(ctx context.Context, config *config.AgentConfig) error {
