...
func NewReviewAgent(config *config.AgentConfig, provider llm.Provider, toolRegistry *tools.ToolRegistry) (*ReviewAgent, error) {
	if provider == nil {
	...
	baseAgent := agent.NewBaseAgent("review-agent", "Review Agent", config)
	return &ReviewAgent{
		BaseAgent:    baseAgent,
...
func (a *ReviewAgent) Initialize(ctx context.Context, config *config.AgentConfig) error {
